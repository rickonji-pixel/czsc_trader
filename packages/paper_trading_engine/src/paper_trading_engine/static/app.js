...
export const auditScopeLabel = (event,accounts=[]) => {
  ...
  if(event?.channel==='futu')return accountLabel?`Futu模拟渠道 · 关联虚拟账户 ${accountLabel}`:'Futu模拟渠道';
  if(event?.channel==='virtual'&&accountLabel)return `虚拟账户 · ${accountLabel}`;
  if(accountLabel)return `账户 · ${accountLabel}`;
  return '历史记录 · 作用域未记录';
};
...
export const channelOrderAccountLabel = order => order?.virtual_account_id||'历史未记录';
...
function renderChannel(s){document.querySelector('#app').innerHTML=`<div class="workspace"><section class="panel hero"><div><div class="eyebrow">执行渠道 · FUTU</div><h1>Futu 模拟账户</h1><div class="subtle">${esc(s.scope.account_type)} · 行情 ${statusLabel(s.quote_health)} · 最近同步 ${esc(formatBeijingTime(s.as_of))}</div></div><button id="channelSwitch" class="switch ${s.paused?'paused':''}" role="switch" aria-checked="${!s.paused}">${s.paused?'渠道已暂停 · 点击恢复':'渠道自动运行 · 点击暂停'}</button></section>${alerts((s.alerts||[]).map(x=>({message:x})))}<div class="grid">${metric('可用现金',money(s.account?.cash))}${metric('总资产',money(s.account?.total_assets))}${metric('实际持仓',`${Number(s.actual_quantity||0).toLocaleString()} 股`)}${metric('连接状态',s.connection_error?'不可用':'已连接')}</div><section class="panel section"><div class="section-head"><h2>订单与虚拟账户</h2><span class="badge">逐笔审计</span></div>${rowsTable([['时间',r=>formatBeijingTime(r.created_at)],['Futu订单',r=>r.channel_order_id],['虚拟账户',r=>channelOrderAccountLabel(r)],['策略版本',r=>r.strategy_release_id||'历史未记录'],['决策ID',r=>r.decision_id||'历史未记录'],['方向',r=>r.side],['数量',r=>r.quantity],['价格',r=>r.limit_price],['状态',r=>statusLabel(r.status)],['操作',r=>['SUBMITTED','PARTIAL_FILLED'].includes(r.status)?'二次确认撤单':'—']],s.orders)}</section></div>`;document.querySelector('#channelSwitch').onclick=async()=>{try{await post(`/api/channels/futu/${s.paused?'resume':'pause'}`);toast(s.paused?'渠道已恢复':'渠道已暂停');await loadRoute({showLoading:false});}catch(e){toast(e.message);}};document.querySelectorAll('tbody tr').forEach((tr,i)=>{const o=s.orders?.[i];if(o&&['SUBMITTED','PARTIAL_FILLED'].includes(o.status))tr.onclick=()=>cancelOrder(o);});}
async function cancelOrder(order){if(!confirm(`确认申请撤销Futu订单？\n订单 ${order.channel_order_id}\n${order.side} ${order.quantity}股\n虚拟账户 ${channelOrderAccountLabel(order)}\n策略 ${order.strategy_release_id||'历史未记录'}`))return;try{const {token}=await post('/api/channels/futu/cancel-token',{channel_order_id:order.channel_order_id});if(!confirm('撤单令牌已签发，有效期2分钟。再次确认撤单。'))return;await post('/api/channels/futu/cancel',{channel_order_id:order.channel_order_id,token});toast('撤单请求已提交');await loadRoute({showLoading:false});}catch(e){toast(e.message);}}
