...
EXPERIMENT_ID = "20260910_S002_EX11"
...
def _validate_sources(repo_root: Path, protocol: dict[str, object]) -> None:
    ...
    failed = repo_root / "experiments" / specs["failed_attempt"]["experiment_id"]
    ...
    expected = {
        failed / "experiment_manifest.json": specs["failed_attempt"]["manifest_sha256"],
        failed / "artifacts" / "protocol.json": specs["failed_attempt"]["protocol_sha256"],
        failed / "artifacts" / "512100_adjusted_daily.csv": specs["failed_attempt"]["adjusted_daily_sha256"],
        failed / "artifacts" / "512100_execution_daily.csv": specs["failed_attempt"]["execution_daily_sha256"],
        failed / "artifacts" / "512100_data_manifest.json": specs["failed_attempt"]["data_manifest_sha256"],
...
def _prepare_or_load_validation_data(
    ...
    if not manifest_path.is_file():
        raise FileNotFoundError("frozen EX10 validation-data snapshot is missing")
    manifest = _read_json(manifest_path)
    failed = protocol["source_experiments"]["failed_attempt"]
    expected = {
        adjusted_path: failed["adjusted_daily_sha256"],
        execution_path: failed["execution_daily_sha256"],
        manifest_path: failed["data_manifest_sha256"],
    }
    for path, digest in expected.items():
        if _sha256(path) != digest:
            raise ValueError(f"frozen validation-data snapshot differs: {path.name}")
...
def main() -> None:
    ...
    execution_prices = adjusted.set_index("dt").sort_index()
    ...
    metrics = _strategy_metrics(states, adjusted, protocol)
    ...
    (experiment_dir / "03_execution.md").write_text(
        f"# {EXPERIMENT_ID} 执行\n\n状态：COMPLETE。\n\n"
        f"复用EX10在规则冻结后取得的512100日线快照，并校验全部来源哈希。"
        f"收益核算采用后复权连续开盘价，正确跨越2022-09-05份额折算；未复权价格仅作审计。"
