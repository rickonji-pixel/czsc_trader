...
function renderChannel(s){document.querySelector('#app').innerHTML=`<div class="workspace"><section class="panel hero"><div><div class="eyebrow">执行渠道 · FUTU</div><h1>Futu 模拟渠道</h1><div class="subtle">底层模拟账户 · 对账 ${statusLabel(s.reconciliation_status)} · 最近同步 ${esc(formatBeijingTime(s.as_of))}</div></div><button id="channelSwitch" class="switch ${s.paused?'paused':''}" role="switch" aria-checked="${!s.paused}">${s.paused?'渠道已暂停 · 点击恢复':'渠道自动运行 · 点击暂停'}</button></section>${alerts((s.alerts||[]).map(x=>({message:x})))}<div class="grid">${metric('可用现金',money(s.account?.cash))}${metric('总资产',money(s.account?.total_assets))}${metric('已分配资金',money(s.allocated_capital))}${metric('未分配资金',money(s.unallocated_capital))}</div><section class="panel section"><div class="section-head"><h2>承载账户</h2><span class="badge">${(s.accounts||[]).length} 个账户</span></div>${rowsTable([['虚拟账户',r=>r.name],['账户ID',r=>r.account_id],['策略版本',r=>r.release_id],['交易标的',r=>r.symbol],['初始资金',r=>money(r.initial_cash)],['可用现金',r=>money(r.cash)],['持仓',r=>r.quantity],['状态',r=>r.paused?'已暂停':statusLabel(r.status)]],s.accounts)}</section><section id="channelOrders" class="panel section"><div class="section-head"><h2>订单</h2><span class="badge">逐笔归属</span></div>${rowsTable([['时间',r=>formatBeijingTime(r.created_at)],['Futu订单',r=>r.channel_order_id],['虚拟账户',r=>channelOrderAccountLabel(r)],['决策ID',r=>r.decision_id||'历史未记录'],['方向',r=>r.side],['数量',r=>r.quantity],['委托价',r=>orderPriceLabel(r)],['状态',r=>statusLabel(r.status)],['操作',r=>['SUBMITTED','FILLED_PART'].includes(r.status)?'二次确认撤单':'—']],s.orders)}</section><section class="panel section"><div class="section-head"><h2>成交</h2><span class="badge">Futu回报</span></div>${rowsTable([['时间',r=>formatBeijingTime(r.occurred_at)],['虚拟账户',r=>channelOrderAccountLabel(r)],['成交ID',r=>displayFillId(r.fill_id),r=>r.fill_id],['Futu订单',r=>r.order_id],['方向',r=>r.side],['数量',r=>r.quantity],['价格',r=>r.price],['费用',r=>r.fee]],s.fills)}</section></div>`;document.querySelector('#channelSwitch').onclick=async()=>{try{await post(`/api/channels/futu/${s.paused?'resume':'pause'}`);toast(s.paused?'渠道已恢复':'渠道已暂停');await loadRoute({showLoading:false});}catch(e){toast(e.message);}};document.querySelectorAll('#channelOrders tbody tr').forEach((tr,i)=>{const o=s.orders?.[i];if(o&&['SUBMITTED','FILLED_PART'].includes(o.status))tr.onclick=()=>cancelOrder(o);});}
