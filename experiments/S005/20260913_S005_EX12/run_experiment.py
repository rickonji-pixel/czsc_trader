...
EXPERIMENT_ID = "20260913_S005_EX12"
...
def _daily(repo: Path, cutoff: pd.Timestamp) -> pd.DataFrame:
    ...
    if not paths:
        raise FileNotFoundError("minute research data unavailable: 588080.SH")
...
def main() -> None:
    ...
    validate_experiment_archive(repo / "experiments/S005/20260913_S005_EX11")
    ...
    frame["true_range_pct"] = ranges.max(axis=1) / previous_close
    ...
    frame = frame.loc[
        (frame.index >= pd.Timestamp(str(protocol["development_start"]))) & (frame.index <= cutoff)
    ].copy()
    ...
    for quantile in quantiles:
        name = f"DOWNSIDE_CAPITULATION_Q{int(quantile * 100)}"
        threshold = frame[f"q{int(quantile * 100)}"]
        raw = (
            (frame.true_range_pct >= threshold)
            & (frame["return"] < 0)
            & (frame.close_location <= float(protocol["maximum_close_location"]))
        )
    ...
    decision = "PROCEED_TO_FIXED_THREE_SESSION_RETURN_TEST" if selected else "STOP_DOWNSIDE_CAPITULATION_ON_DENSITY"
    compression = {"method": "gzip", "compresslevel": 9, "mtime": 0}
    ledger.reset_index(names="date").to_csv(
        artifacts / "event_ledger.csv.gz", index=False, compression=compression, lineterminator="\n"
    )
