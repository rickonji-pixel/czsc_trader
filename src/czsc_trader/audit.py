...
def audit_candidate_evaluation(
    ...
    """Adapt Trader frames to the SE-owned execution audit."""
    index = pd.DatetimeIndex(pd.to_datetime(target_position.index), name="dt")
    dates = tuple(timestamp.date().isoformat() for timestamp in index)
    events = tuple(
        FactorEvent(
            str(event.get("event_id", "")), pd.Timestamp(event["signal_date"]).date().isoformat(),
            str(event["event_type"]), float(event["before_position"]),
            float(event["after_position"]),
            None if event.get("factor_score") is None else float(event["factor_score"]),
        )
    ...
    for result in period_results.values():
        ...
        for order in frame.to_dict("records"):
            signal_date = pd.Timestamp(order["signal_date"])
            execution_date = pd.Timestamp(order["execution_date"])
            orders.append(ExecutionOrder(
                signal_date.date().isoformat(), execution_date.date().isoformat(),
                str(order["side"]), float(order["price"]), float(order["size"]),
                float(order["fees"]), str(order.get("factor_event_id", "")),
                execution_date == period_start and signal_date < period_start and order["side"] == "Buy",
            ))
    finding = audit_execution(ExecutionEvidence(
        dates, tuple(map(float, target_position)), tuple(map(float, scores)),
        events, tuple(orders), "0" * 64, len(period_results),
    ))
    if finding.status.value != "PASS":
        raise AssertionError(", ".join(finding.reason_codes))
    return {"status": finding.status.value, **dict(finding.details)}
