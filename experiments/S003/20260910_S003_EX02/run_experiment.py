...
EXPERIMENT_ID = "20260910_S003_EX02"
...
def main() -> None:
    ...
    evidence = {
        ...
        "manifest_sha256": raw_file_sha256(
            data_dir / "510500_intraday_manifest.json"
        ),
    ...
    (experiment / "04_conclusion.md").write_text(
        f"# {EXPERIMENT_ID} 结论\n\n"
        "S003所需的完整分钟研究数据已就绪。三个周期覆盖相同交易日并完成日线对账，可以"
        "进入日线regime定义与15m CZSC信号普查。分钟K线数量不作为独立样本数，后续仍按"
        "独立交易事件和交易日聚类评估。\n",
        encoding="utf-8",
    )
