...
EXPERIMENT_ID = "20260923_S008_EX26"
...
def _evaluate_worker(task: Mapping[str, Any]) -> dict[str, object]:
    ...
    candidate = _candidate(
        ...
        prototype_id,
        f"EX26P{int(task['prototype_ordinal']):02d}T{trial_number:06d}",
        rule,
    )
...
def main() -> None:
    ...
    if not protocol.get("reads_real_returns") or not protocol.get("starts_search"):
        raise ValueError("EX26 must explicitly declare formal return search")
    prohibited = (
        "selects_winner",
    ...
    if any(protocol.get(key) for key in prohibited):
        raise ValueError("EX26 scope exceeds formal development-pool search")
    ...
    if (
        ...
        raise ValueError("EX26 in-memory, eight-core sequential execution differs")
    ...
    if _sha256(runtime_path) != sources["runtime_raw_sha256"]:
        raise ValueError("EX26 runtime raw source differs")
    ...
    if runtime_identity != sources["runtime_implementation_sha256"]:
        raise ValueError("EX26 runtime implementation identity differs")
    ...
    if prototype_order != list(registered):
        raise ValueError("EX26 prototype order differs from EX19/EX25")
    ...
    prepared_root = (tmp_parent / "S008_EX26_prepared").resolve()
    if prepared_root.parent != tmp_parent:
        raise ValueError("unsafe EX26 temporary directory")
    if prepared_root.exists():
        raise ValueError("EX26 temporary directory already exists; refusing an implicit resume")
    ...
    bundle = {
        ...
        "worker_data_dir": str(tmp_parent / "S008_EX26_worker"),
    }
    _worker_init(bundle)
                ...
                for item in sorted(batch, key=lambda value: value["trial"].number):
                    ...
                    if failures:
                        ...
                        try:
                            ...
                            raise RuntimeError(
                                f"EX26 execution failed: {prototype_id} trial {trial.number}"
                            ) from exc
                    rows.append(row)
                _checkpoint(ledger_path, rows)
    ...
    if len(ledger) != int(search_contract["total_trials"]):
        raise ValueError("EX26 trial ledger count differs from frozen budget")
    if ledger["trial_state"].eq("EXECUTION_FAILED").any():
        raise ValueError("EX26 contains an execution failure")
