...
def _monthly_causal_cpi(frame: pd.DataFrame, sessions: pd.DatetimeIndex) -> pd.Series:
    ...
    values = values.loc[values["Date"] >= sessions[0]].copy()
    ...
    if effective.index.duplicated().any():
...
class S008Prototype(StrategyImplementation):
    ...
    def calculate_history(
        ...
        unexpected = missing[missing >= history.index.min()]
        if not unexpected.empty:
            raise RuntimeContractError(
                "S008 prototype history misses calculation sessions: "
                + ",".join(item.date().isoformat() for item in unexpected)
            )
        result = history.reindex(requested)
        if not missing.empty:
            result.loc[missing, "target_position"] = 0.0
            result.loc[missing, "action"] = "HOLD_CASH"
            result.loc[missing, "prototype_id"] = str(self._parameters["prototype_id"])
        return result
