...
模板定义位于仓库根目录 `strategy_templates/templates.json`。TDR 提供查询入口：

```powershell
czsc-trader template validate
czsc-trader template list --status READY
czsc-trader template show --id STC-T04-EVENT-HOLD
czsc-trader template instantiate --spec .\prototype.json
```
