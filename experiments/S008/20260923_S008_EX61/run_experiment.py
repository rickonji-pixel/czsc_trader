...
EXPERIMENT_ID = "20260923_S008_EX61"
...
def _evaluate_worker(task: Mapping[str, Any]) -> dict[str, object]:
    ...
    candidate = _candidate(
        ...
        f"EX61T{int(task['trial_number']):06d}",
        parameters,
    )
...
def _safe_reset(path: Path, parent: Path) -> None:
    target = path.resolve()
    root = parent.resolve()
    if target.parent != root or not target.name.startswith("S008_EX61_"):
        raise ValueError("unsafe EX61 temporary path")
    if target.exists():
...
def main() -> None:
    ...
    runtime_root = repo / "experiments/S008/20260923_S008_EX59/runtime/strategy_runtime"
    ...
    prepared_root = tmp_parent / "S008_EX61_prepared"
    _safe_reset(prepared_root, tmp_parent)
    anchor_candidate = _candidate(runtime_root, runtime_identity, "EX61ANCHOR", _anchor())
    ...
    bundle = {
        ...
        "worker_data_dir": str(tmp_parent / "S008_EX61_worker"),
    }
    _worker_init(bundle)
    ...
    buyhold_candidate = _candidate(runtime_root, runtime_identity, "EX61BUYHOLD", _anchor())
    baseline = {
            ...
            for trial, parameters, failures in sorted(batch, key=lambda item: item[0].number):
                if failures:
                    ...
                    try:
                        ...
                        raise RuntimeError(f"EX61 trial {trial.number} failed") from exc
                rows.append(row)
            _checkpoint(ledger_path, rows)
