...
完整评价合同必须逐项声明目标复算、频率复算、参数稳健性、统计稳健性、成本压力、技术回放、
外部验证、运行时验收和监测方案。TDR只接受截至开发截止交易日的完整数据，正式复算禁用评价
缓存及实验制品复用，并把候选执行契约和`TXE-v1`成交语义写入裁决证据。任何缺项、截止不足、
载荷声明与SRT执行规则不一致都会阻断裁决。
...
- 候选提交、TDR裁决和人工批准三枚前置印章的哈希；
- SRT可执行身份和release hash。

最终`VERSION_FROZEN`印章反向固化完整版本记录哈希、release hash、证据ID和生命周期事件ID，
从而避免版本记录与最终印章相互包含造成循环哈希。版本号在冻结时分配，不为尚未冻结的候选
预占`vN`。SM必须验证完整SGC哈希链、冻结印章、版本记录哈希和release hash；任何一处缺失
或漂移均不得部署。历史版本继续沿用原有release hash算法，不重写既有身份。
...
同一StrategyFamily可以继续启动新的研究批次。首批次默认使用`SGC-SXXX-001`；后续批次必须
显式提供新的凭据ID，例如`SGC-SXXX-002`，并在同一研究目录的`batches/`下保存立项摘要。
