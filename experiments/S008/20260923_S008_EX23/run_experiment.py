...
EXPERIMENT_ID = "20260923_S008_EX23"
...
def _managed_directory(*, prefix: str, dir: Path):
    ...
    path = (root / "S008_EX23_runtime").resolve()
    if path.parent != root or path.name != "S008_EX23_runtime":
        raise ValueError("unsafe EX23 temporary path")
    if path.exists():
        shutil.rmtree(path)
...
def main() -> None:
    ...
    predecessor = repo / "experiments/S008/20260923_S008_EX22"
    ...
    paths = {
        "ex22_manifest_sha256": predecessor / "experiment_manifest.json",
    ...
    for name in ("03_execution.md", "04_conclusion.md"):
        ...
        path.write_text(
            path.read_text(encoding="utf-8").replace("# S008 EX20", "# S008 EX23"),
            encoding="utf-8",
        )
