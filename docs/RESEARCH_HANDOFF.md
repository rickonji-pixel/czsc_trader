...
2026-08-23 将588080本地行情统一重建为 Tushare `fund_adj` 后复权数据，并针对 Tushare 在588080和515050各自白名单内的7个已确认2024年异常交易日修正100倍的30分钟成交量。日期均为 `04-03`、`04-19`、`04-26`、`04-30`、`05-24`、`05-31`、`06-14`；其他证券和日期不做启发式修正。冻结规则未改变，仍登记为 `baseline_20260823`。
...
2026-08-23 使用 `baseline_20260823` 对 `515050.SH` 执行固定规则回测，窗口为2026-06-24至2026-08-21。本地输出为：

```text
D:\CodeBase\czsc_trader\outputs\515050_0823_R01
```

数据准备覆盖2024-01-02至2026-08-21，使用 Tushare `fund_adj` 后复权数据，共发布9个年度行情文件：30分钟5,112根、日线639根、周线136根，manifest和跨周期验证均为 `PASS`。515050在上述7个2024年交易日同样存在分钟成交量恰好放大100倍的问题，OHLC完全一致、成交额仅有舍入级误差；修正采用显式证券代码/日期白名单并记录在manifest。相关数据与修正已由提交 `4ea4532` 合入master。

| 指标 | 基线策略 | Buy & Hold | 差异 |
| --- | ---: | ---: | ---: |
| 收益率 | -16.35% | -20.85% | +4.50个百分点 |
| 夏普率 | -1.776 | -1.720 | -0.056 |
| 最大回撤 | -32.12% | -38.78% | +6.65个百分点 |
| 时间持仓比例 | 55.81% | 100.00% | -44.19个百分点 |

基线共产生6笔订单、3个完整回合，策略在前两轮亏损、第三轮盈利。无前视审计为 `PASS`，检查6笔订单、6个来源事件和639个每日仓位。由于未提供收益目标，程序验收状态为 `N/A`；研究判断明确为 `FAIL`：策略虽然少亏并降低回撤，但仍亏损16.35%，且夏普率为负并略差于Buy & Hold，不能视为有效盈利策略。

55.81%表示43个交易日中有24日全仓持有、19日空仓，不表示每次只投入55.81%资金。首日根据2026-06-23已经形成的状态在6月24日开盘全仓买入。当前 `equity.csv` 的首日 `execution_position` 导出为空，是展示层没有写入首日初始仓位的已知缺陷；实际回测、收益和持仓比例均已按首日全仓计算，缺陷尚未修复。
