...
EXPERIMENT_ID = "20260923_S008_EX63"
...
def _safe_reset(path: Path, parent: Path) -> None:
    target = path.resolve()
    root = parent.resolve()
    if target.parent != root or target.name != "S008_EX63_prepared":
...
def main() -> None:
    ...
    predecessor = repo / "experiments/S008/20260923_S008_EX62"
    runtime_root = experiment / "runtime/strategy_runtime"
    runtime_path = runtime_root / "strategies/s008_precious_metal_preference.py"
    if _sha256(predecessor / "experiment_manifest.json") != protocol["sources"]["ex62_manifest_sha256"]:
        raise ValueError("EX62 manifest differs from frozen source")
    if _sha256(runtime_path) != protocol["sources"]["runtime_file_sha256"]:
        raise ValueError("EX63 runtime file differs from frozen source")
    ...
    if runtime_identity != protocol["sources"]["runtime_source_sha256"]:
        raise ValueError("EX63 runtime closure differs from frozen source")
    if any(
        protocol.get(key)
    ...
    prepared_root = tmp_parent / "S008_EX63_prepared"
