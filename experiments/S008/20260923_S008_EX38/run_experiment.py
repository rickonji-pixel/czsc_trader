...
EXPERIMENT_ID = "20260923_S008_EX38"
...
def _evaluate_worker(task: Mapping[str, Any]) -> dict[str, object]:
    ...
    candidate = _candidate(
        ...
        prototype_id,
        f"EX38P{int(task['prototype_ordinal']):02d}T{trial_number:06d}",
        rule,
    )
...
def main() -> None:
    ...
    if not protocol.get("reads_real_returns") or not protocol.get("starts_search"):
        raise ValueError("EX38 must explicitly declare formal return search")
    prohibited = (
        "selects_winner",
    ...
    if any(protocol.get(key) for key in prohibited):
        raise ValueError("EX38 scope exceeds formal development-pool search")

    sources = dict(protocol["sources"])
    frozen_paths = {
        repo / sources["ex37_manifest_path"]: sources["ex37_manifest_sha256"],
        repo / sources["ex36_manifest_path"]: sources["ex36_manifest_sha256"],
    ...
    ex37_archive = repo / str(sources["ex37_manifest_path"]).replace(
        "/experiment_manifest.json", ""
    )
    ex36_archive = repo / str(sources["ex36_manifest_path"]).replace(
        "/experiment_manifest.json", ""
    )
    ...
    validate_experiment_archive(ex37_archive)
    validate_experiment_archive(ex36_archive)
    ...
    if (
        ...
        raise ValueError("EX38 in-memory, eight-core sequential execution differs")
    ...
    if _sha256(runtime_path) != sources["runtime_raw_sha256"]:
        raise ValueError("EX38 runtime raw source differs")
    ...
    if runtime_identity != sources["runtime_implementation_sha256"]:
        raise ValueError("EX38 runtime implementation identity differs")
    runtime = _load_runtime(runtime_path, "s008_ex38_main_runtime")
    ...
    if prototype_order != list(registered):
        raise ValueError("EX38 prototype order differs from EX19/EX25")
    ...
    prepared_root = (tmp_parent / "S008_EX38_prepared").resolve()
    if prepared_root.parent != tmp_parent:
        raise ValueError("unsafe EX38 temporary directory")
    if prepared_root.exists():
        raise ValueError("EX38 temporary directory already exists; refusing an implicit resume")
    ...
    bundle = {
        ...
        "worker_data_dir": str(tmp_parent / "S008_EX38_worker"),
    }
    _worker_init(bundle)
    ...
    evaluation_instance = StrategyRuntime().create(
        StrategyInit(
            anchor_candidate,
    ...
    evaluation_summary = evaluation_instance.prepare_data()
    data_identity["evaluation_prepared_data_identity"] = evaluation_summary.data_identity
    full_executor = HistoricalExecutor(
        strategy_reference=evaluation_instance.definition.release_id,
        symbol=evaluation_instance.identity.symbol,
        execution_daily=execution_daily,
        execution_intraday=pd.DataFrame(columns=["dt", "high", "low"]),
        evaluation_start=evaluation_start,
        evaluation_end=evaluation_end,
        initial_cash=_INITIAL_CASH,
        execution_policy=evaluation_instance.execution_policy,
        order_types=evaluation_instance.definition.capabilities.order_types,
        account_id="EX38-anchor-full",
    )
    full_anchor_account = evaluation_instance.run_window(executor=full_executor)
                ...
                for item in sorted(batch, key=lambda value: value["trial"].number):
                    ...
                    if failures:
                        ...
                        try:
                            ...
                            raise RuntimeError(
                                f"EX38 execution failed: {prototype_id} trial {trial.number}"
                            ) from exc
                    rows.append(row)
                _checkpoint(ledger_path, rows)
    ...
    if len(ledger) != int(search_contract["total_trials"]):
        raise ValueError("EX38 trial ledger count differs from frozen budget")
    if ledger["trial_state"].eq("EXECUTION_FAILED").any():
        raise ValueError("EX38 contains an execution failure")
