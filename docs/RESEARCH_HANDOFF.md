...
- 正式发布哈希：`146490c8a4ee75f307d3d59453efaebc8fea671c726c023c96449706c40b944e`
- 策略类型：`czsc_regime_weight`
- 信号来源：`experiments/0903_EX06`候选R1102
- 人工冻结：2026-09-03，接受记录见`experiments/0903_EX06/evaluation_acceptance.json`
...
- 当前行情截止：2026-09-02
- 已冻结实验档案：56个，截止 `0903_EX06`

`S001-v2`的策略选择数据截止到2026-09-02，2026年历史数据已经参与候选比较，因此
不能整体视为独立样本外。`S001-v2`从2026-09-03之后产生的首个新信号开始形成前瞻
运行记录；这部分记录用于观察策略、执行和系统可靠性，在积累出足够样本前不宣称
样本外或实盘有效性。项目尚未引入真实资金成交反馈。

## 当前正式策略及研究来源

当前正式模拟观察身份是`S001-v2`，研究来源为候选R1102。原版本`S001-v1`仍是
不可变历史版本；“候选143”“冠军策略”和`baseline_20260903`分别是其研究来源称呼
和历史兼容别名，不再承担正式命名。`S001-v1`的信号部分来自
...
新研究采用`opc-v3`：确定性筛选和排名后，SE统一完成既有执行、重复性、账本和压力
审计，并增加CSCV/PBO、Deflated Sharpe、配对平稳区块Bootstrap及真实候选参数邻域
检验。统计结果只标记`FAVORABLE/MIXED/WEAK`，不承担自动否决；研究者根据完整证据
人工确认是否冻结。2026-09-02及以前仍属于开发池，冻结后的下一笔新信号才进入前瞻
观察。
...
`0903_EX04`把上述1,187项候选放入统一Strategy Evaluator，以全期、五个完整自然年和
2026 YTD执行OPC-v1非劣筛选。旧候选1010因2026 YTD净CAGR、最大回撤和卡玛均违反
边界，没有进入正式短名单。8项正式挑战者全部被淘汰，结论为
`KEEP_INCUMBENT / NO_ELIGIBLE_CHALLENGER`，因此EX03提出的1010冻结建议已被统一评价
结论取代。

最接近的R0539在完整执行规则下把全期净CAGR从22.83%提高到27.36%、最大回撤从
-23.93%改善到-20.19%、卡玛从0.9539提高到1.3549、盈亏因子从2.5592提高到
3.0492，纯range收益从-25.14%改善到-8.65%。它仅因2021和2023卡玛仍为负而不符合
OPC-v1。当前研究建议是保留`S001-v1`，下一轮围绕R0539与R1297等互补种子搜索同时
跨越2021、2023零线的局部平台。机器证据位于`experiments/0903_EX04/artifacts/`。

`0903_EX05`将负卡玛窗口改为相对基线非劣的OPC-v2，并完整重算1,187项候选。10项候选
进入排名，R1102成为唯一临时冠军；R0539与R1319同在Pareto第一层。该轮建议冻结
R1102，但原冠军体检缺少统计稳健性审计，因此没有执行人工接受。

`0903_EX06`保持EX05候选payload、试验台账和开发池不变，使用OPC-v3补齐完整审计。
R1102仍是唯一临时冠军，工程审计与邻域数据完整性均通过，最终风险标签为`MIXED`：
Sharpe-PBO为0.1190，配对Bootstrap方向偏正面；DSR原始/有效试验数概率为0.2835/
0.7262，真实候选邻域方向偏弱。SE继续给出`RECOMMEND_FREEZE`。用户于2026-09-03
人工接受该建议，R1102已经冻结为`S001-v2`（资格`PAPER_READY`），并以10万元初始资金
注册PTE虚拟账户`s001-v2`。统计风险`MIXED`作为披露标签保留，不构成自动否决。机器
证据位于`experiments/0903_EX06/artifacts/`，接受旁证位于该实验根目录。
...
SQLite审计和本机观测页面。它同时维护相互隔离的本地虚拟账户；`s001-v1`绑定
`S001-v1`，`s001-v2`绑定`S001-v2`，两个账户名称统一为“发布ID+模拟账户”，初始
资金均为10万元。当前外部渠道为Futu模拟交易，
PTE与Trader是同仓库并列包；
...
7. 新实验先读取正式策略版本及其历史来源，再使用当日尚未占用的下一个
   `MMDD_EXXX`编号；当前最后一个完成档案为`0903_EX04`。独立执行规则注册表只服务历史复现。
