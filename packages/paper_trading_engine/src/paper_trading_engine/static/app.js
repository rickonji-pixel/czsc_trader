...
export function channelMarkup(s){
  ...
  return `<div class="workspace"><section class="panel hero"><div><div class="eyebrow">执行渠道 · FUTU 模拟盘 CN</div><h1>Futu 模拟盘 CN 渠道</h1><div class="subtle">底层模拟账户 · 对账 ${statusLabel(s.reconciliation_status)} · 最近对账 ${esc(formatBeijingTime(s.last_reconcile_at))}</div></div><button id="channelSwitch" class="switch ${s.paused?'paused':''}" role="switch" aria-checked="${!s.paused}">${s.paused?'渠道已暂停 · 点击恢复':'渠道自动运行 · 点击暂停'}</button></section>${alerts((s.alerts||[]).map(x=>({message:statusLabel(x)})))}<div class="grid channel-core-metrics">${metric('Futu总资产',money(s.account?.total_assets))}${metric('可用现金',money(s.account?.cash))}${metric('已分配额度',money(s.strategy_allocated_capital))}${metric('未分配额度',money(s.unallocated_capital))}</div><section class="panel reconciliation-summary"><div class="reconciliation-bar"><div class="reconciliation-state ${cashOk?'ok':'warning'}"><strong>${cashState}</strong><span>现金差异 ${money(s.cash_difference)}</span></div><div class="reconciliation-state ${assetOk?'ok':'neutral'}"><strong>${assetState}</strong><span>Futu−PTE ${money(s.asset_difference)}</span></div><details class="reconciliation-details"><summary>查看明细</summary><div class="reconciliation-detail-grid"><div><span>Futu持仓市值</span><strong>${money(s.account?.market_value)}</strong></div><div><span>PTE账务现金</span><strong>${money(s.logical_cash)}</strong></div><div><span>PTE日终总资产</span><strong>${money(s.logical_total_assets)}</strong></div><div><span>Futu−PTE估值差异</span><strong>${money(s.asset_difference)}</strong></div><div><span>平账账户余额</span><strong>${money(s.reconciliation_account?.cash)}</strong></div></div><p>Futu按当前行情实时估值；PTE账务总资产按各虚拟账户最近日终数据估值。</p></details></div></section><section class="panel section"><div class="section-head"><h2>策略承载账户</h2><span class="badge">${(s.accounts||[]).length} 个账户</span></div>${rowsTable([['虚拟账户',r=>r.name],['账户ID',r=>r.account_id],['策略版本',r=>r.release_id],['交易标的',r=>r.symbol],['分配额度',r=>money(r.initial_cash)],['可用现金',r=>money(r.cash)],['冻结资金',r=>money(r.frozen_cash)],['持仓',r=>formatQuantity(r.quantity)],['状态',accountOperatingStatus]],s.accounts)}</section><section id="channelOrders" class="panel section"><div class="section-head"><h2>订单</h2><span class="badge">逐笔归属</span></div>${rowsTable([['时间',r=>formatBeijingTime(r.created_at)],['Futu订单',r=>r.channel_order_id],['虚拟账户',r=>channelOrderAccountLabel(r)],['决策ID',r=>r.decision_id||'历史未记录'],['方向',r=>sideLabel(r.side)],['数量',r=>formatQuantity(r.quantity)],['委托价',r=>orderPriceLabel(r)==='市价'?'市价':formatPrice(orderPriceLabel(r))],['状态',r=>statusLabel(r.status)],['操作',r=>['SUBMITTED','FILLED_PART'].includes(r.status)?'二次确认撤单':'—']],s.orders)}</section><section class="panel section"><div class="section-head"><h2>成交</h2><span class="badge">Futu回报</span></div>${rowsTable([['时间',r=>formatBeijingTime(r.occurred_at)],['虚拟账户',r=>channelOrderAccountLabel(r)],['成交ID',r=>displayFillId(r.fill_id),r=>r.fill_id],['Futu订单',r=>r.order_id],['方向',r=>sideLabel(r.side)],['数量',r=>formatQuantity(r.quantity)],['价格',r=>formatPrice(r.price)],['费用',r=>money(r.fee)]],s.fills)}</section></div>`;
}
function renderChannel(s){document.querySelector('#app').innerHTML=channelMarkup(s);document.querySelector('#channelSwitch').onclick=async()=>{try{await post(`/api/channels/futu-simulate-cn/${s.paused?'resume':'pause'}`);toast(s.paused?'渠道已恢复':'渠道已暂停');await loadRoute({showLoading:false});}catch(e){toast(e.message);}};document.querySelectorAll('#channelOrders tbody tr').forEach((tr,i)=>{const o=s.orders?.[i];if(o&&['SUBMITTED','FILLED_PART'].includes(o.status))tr.onclick=()=>cancelOrder(o);});}
