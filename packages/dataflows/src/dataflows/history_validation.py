...
def validate_market_frames(
    ...
    *,
    ...
    """Validate a TDR/SRT market collection and return its stable DFLS report."""

    report = inspect_market_collection(
        {"30m": intraday, "daily": daily, "weekly": weekly},
        execution_daily=execution_daily,
        trading_calendar=trading_calendar,
        expected_start=expected_start,
    )
    report.require_pass()
    intraday_metrics = dict(report.metrics.get("30m", {}))
    weekly_metrics = dict(report.metrics.get("weekly_reconciliation", {}))
    return {
        "status": "PASS",
        "contract": "dfls.history.v1",
        "intraday": {
            "bar_count": intraday_metrics.get("row_count"),
            "complete_day_count": intraday_metrics.get("complete_day_count"),
            "session_times": intraday_metrics.get("session_times", []),
        },
        "reconciliation": {
            "daily_matched_days": intraday_metrics.get("daily_matched_days"),
            "weekly_matched_periods": weekly_metrics.get("weekly_matched_periods"),
            "price_tolerance": intraday_metrics.get("price_tolerance"),
            "volume_relative_tolerance": intraday_metrics.get(
                "volume_relative_tolerance"
            ),
            "amount_relative_tolerance": intraday_metrics.get(
                "amount_relative_tolerance"
            ),
            "calendar_matched_sessions": report.metrics.get(
