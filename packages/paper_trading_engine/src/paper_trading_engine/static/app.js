...
export const auditEventLabel = value => ({
  ...
  ORDER_INTENT_CREATED:'创建订单意图',ORDER_INTENT_RECOVERED:'恢复订单意图',ORDER_SUBMISSION_BLOCKED:'订单提交受阻',ORDER_SUBMITTED:'订单已提交',ORDER_SUBMISSION_FAILED:'订单提交失败',ORDER_REJECTED:'订单被明确拒绝',CANCEL_REQUESTED:'请求撤单',CANCEL_SUCCEEDED:'撤单成功',CANCEL_FAILED:'撤单失败',ORDER_PARTIALLY_FILLED:'订单部分成交',ORDER_FILLED:'订单成交',ORDER_TERMINATED:'订单终止',
...
export const sideLabel = value => ({BUY:'买入',SELL:'卖出'}[String(value||'').toUpperCase()]||value||'—');
...
export const statusLabel = value => ({READY:'就绪',RUNNING:'运行中',OK:'正常',CONNECTED:'已连接',DEGRADED:'降级',PAPER_READY:'获准模拟交易',LIVE_READY:'获准实盘交易',BLOCKED:'阻塞',UNAVAILABLE:'不可用',ACCOUNT_CHART_UNAVAILABLE:'前瞻观察图不可用',CHANNEL_UNAVAILABLE:'Futu不可用',CHANNEL_CASH_MISMATCH:'PTE账务现金与Futu现金不一致',CHANNEL_RECONCILIATION_BLOCKED:'渠道对账阻塞',VIRTUAL_ACCOUNT_BLOCKED:'虚拟账户阻塞',ORDER_SUBMISSION_UNRESOLVED:'存在下单结果待确认',DATA_PUBLICATION_FAILED:'发布数据失败',DATA_PUBLICATION_OVERDUE:'数据发布超期',DECISION_GENERATION_OVERDUE:'决策生成超期',SCHEDULER_STALLED:'调度器停滞',SCHEDULER_OPERATION_FAILED:'调度任务失败',PENDING_SUBMIT:'待提交',SUBMITTING:'提交中',SUBMISSION_UNCERTAIN:'提交结果待确认',SUBMISSION_FAILED:'提交失败',REJECTED:'已拒绝',EXPIRED:'已过期',TIMEOUT:'结果未知',SUBMITTED:'已提交',FILLED_PART:'部分成交',FILLED_ALL:'全部成交',SUBMIT_FAILED:'提交失败',CANCELLED_PART:'部分成交后撤单',CANCELLED_ALL:'已撤销',FAILED:'失败',DISABLED:'已失效',DELETED:'已删除',FILL_CANCELLED:'成交已撤销',PENDING:'待执行'}[value]||value||'—');
