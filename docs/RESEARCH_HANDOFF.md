...
- 标的固定为 `588080.SH`；
- 当前数据覆盖上市日至2026-08-28；
- 截至2026-08-28的数据已经被项目观察，全部属于已见历史证据；
- `0824_EX04`已冻结为当前588080正式基线，不再利用已见历史样本继续回溯修补；
- 新的未观察前向行情从2026-08-28之后开始；
...
实际默认版本由`configs/rule_baselines/registry.json`的`latest`决定。`baseline_20260826`与`experiments/0824_EX04/artifacts/frozen_challenger.json`逐字节相同，是12因子四层策略，允许用于所有已验证标的的普通回测。它的调参与研究证据仍只来自`588080.SH`；其他标的结果不自动构成外推有效性证据。该基线最初从2026-08-26开始积累前向记录，但截至2026-08-28的行情现已被观察；当前新的未观察前向行情只能从2026-08-28之后开始。
...
研究链条可以概括为：0824_EX01—EX04建立并冻结EX04；0824_EX05—EX08扩大因子和搜索规模但未产生更优方案；0825_EX01对EX08候选补测仍全部失败；0825_EX02—EX06完成机制、路径、离场和新表示诊断；0826_EX01未找到跨年稳定的偏离边界；0829_EX01的入场定仓没有改变2026路径；0829_EX02证明持仓期动态变仓会实质改变收益路径，但固定的原分数三区机制只通过一个2026窗口；0830_EX01因技术错误中止，逐字重试的0830_EX02表明固定下行风险压力仓位没有候选同时保持收益并改善回撤；0830_EX03—EX08进一步把风险信息拆成三个可解释家族，只有日内抛压在发现段通过门槛，但其锁定候选在2024—2025验证段同时降低收益并恶化最大回撤，故按预注册规则在访问2026前停止。
...
6. 截至2026-08-28的行情已经在项目层面观察，不得再作为新实验的独立留出；新的未观察前向行情从2026-08-28之后开始。
...
EX08和0825_EX01拒绝“扩大同一搜索或重排同一Trial集合”；0825_EX02说明`0824_EX04`附近是局部平台；0825_EX03—0825_EX06逐步证明局部入退场病灶缺少稳定、低污染的新表示；0826_EX01未找到跨年稳定的简单偏离边界；0829_EX01的入场定仓在2026退化为冠军路径；0829_EX02的动态仓位产生了真实差异，但可见半年仅4/8胜出、平均收益增量为负，且原三个2026窗口只胜一个；0830_EX02的27组下行风险仓位没有方案同时保持收益并改善最大回撤；0830_EX03—EX08把风险信息拆成趋势损伤、连续下跌和日内抛压三个机制，唯一发现段胜者未通过锁定验证。

下一步优先级：

1. `baseline_20260826`保持冻结，不再根据2020—2026已见历史修改12因子、权重、阈值或状态机；
2. `baseline_20260823`只用于显式历史复现，不再参与当前冠军判断；
3. 不依据0829_EX01、0829_EX02、0830_EX02或0830_EX03—EX08结果缩小阈值步长、改变半仓比例、组合三个已失败家族、扩大参数网格或放宽收益约束；这些简单仓位路径均未产生可晋升方案，继续研究应先提出不同且有金融机制依据的新信息或架构；
4. 从2026-08-28之后继续积累前向表现、信号事件、成交和偏差记录；
5. 若未来引入新的信息或仓位架构，必须明确它检验的是不同机制，不能把已见结果反馈为同轮规则；
...
3. 完整阅读0824_EX04、0826_EX01、0829_EX01、0829_EX02和0830_EX01—EX08的目标、设计、执行、结论及manifest，确认当前基线来源、停止回溯修补的依据、仓位实验结果以及两条ERROR到等协议重试FAIL的链路。
...
重要边界：当前身份只认configs/rule_baselines/registry.json；baseline_20260826是通用活动正式基线，与0824_EX04冻结策略同源，但调参与研究证据只来自588080；baseline_20260823只用于显式历史复现。研究事实只认Git跟踪的实验档案；引用某轮精确事实前必须完整阅读该轮档案。outputs只用于当前设备新运行的普通回测，隔离回放产物也不是研究证据。截至2026-08-28的数据已经被观察，不能再作为新的独立留出；新的未观察前向行情从2026-08-28之后开始。
