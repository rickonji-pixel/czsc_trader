...
4. 委托受理不代表成交。只有Futu明确返回的累计成交增量能够改变账户现金与持仓；没有
   明确成交回报时维持原账本。订单意图使用原子状态迁移防止并发重复提交；明确拒单、
   提交结果未知和券商受理后的本地落库失败必须分别处理。
...
10. WDG只管理PTE进程生命周期，并通过`/api/system/status`确认进程为`RUNNING`。
   数据发布时间、OpenD连接、交易标的与交易规则均由PTE持有；WDG不得用业务状态判定
   进程健康，也不得在启动PTE时注入业务参数。OpenD晚于PTE启动时，由PTE在后续调度
   周期内重新连接，不重启WDG。
