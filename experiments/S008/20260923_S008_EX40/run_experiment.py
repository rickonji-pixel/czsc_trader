...
EXPERIMENT_ID = "20260923_S008_EX40"
...
def main() -> None:
    ...
    prohibited = (
        "reads_new_labels", "prototype_instantiation", "starts_search",
        "candidate_generation", "reads_sealed_validation", "mutates_catalog",
        "mutates_platform", "mutates_pte",
    )
    if any(protocol.get(key) for key in prohibited):
        raise ValueError("risk component review scope exceeds contract")

    ex39 = repo / "experiments/S008/20260923_S008_EX39"
    ex16 = repo / "experiments/S008/20260923_S008_EX16"
    validate_experiment_archive(ex39)
    validate_experiment_archive(ex16)
    source_paths = {
        "ex39_manifest_sha256": ex39 / "experiment_manifest.json",
        "primary_supported_sha256": ex39 / "artifacts/primary_supported_paths.csv",
    ...
    if (
        ...
        raise ValueError("supported risk path identity differs from protocol")
    ...
    if correlation.isna().all(axis=None):
        raise ValueError("risk component correlation matrix is empty")
    ...
    if sorted(name for group in components for name in group) != sorted(supported["feature"]):
        raise ValueError("redundancy clusters do not cover every supported risk path")
    ...
    for ordinal, members in enumerate(components, start=1):
        ...
        ranked = by_feature.loc[members].sort_values(
            ["evidence_rank", "global_bh_qvalue", "confirmation_residual_ic",
             "bootstrap_positive_probability"],
            ascending=[False, True, False, False], kind="stable",
        )
        best = ranked.iloc[0]
        ties = ranked.loc[
            ranked["evidence_rank"].eq(best["evidence_rank"])
            & ranked["global_bh_qvalue"].eq(best["global_bh_qvalue"])
            & ranked["confirmation_residual_ic"].eq(best["confirmation_residual_ic"])
            & ranked["bootstrap_positive_probability"].eq(best["bootstrap_positive_probability"])
        ]
        ...
        maximum_internal = (
            0.0
            if len(members) == 1
            else float(np.nanmax(np.abs(correlation.loc[members, members].to_numpy() - np.eye(len(members)))))
        )
        cluster_rows.append({
            "cluster_id": cluster_id, "member_count": len(members),
            "members": " | ".join(members), "representative": representative,
        ...
        for member in members:
            ...
            ledger_rows.append({
                ...
                "representative": representative, "selected": member == representative,
                ...
                "audit_role": row["audit_role"], "evidence_label": row["evidence_label"],
    ...
    component_panel = component_panel.sort_values(
        ["hypothesis_id", "information_family", "feature"]
    ).reset_index(drop=True)
    role_counts = Counter(component_panel["audit_role"])
    hypothesis_counts = Counter(component_panel["hypothesis_id"])
    family_counts = Counter(component_panel["information_family"])
    required_roles = set(protocol["required_audit_roles"])
    observed_roles = set(component_panel["audit_role"])
    passes = bool(
    ...
    decision = "PROCEED_TO_P04_PREREGISTRATION" if passes else "STOP_RISK_COMPONENT_PANEL_INCOMPLETE"
    pd.DataFrame(cluster_rows).to_csv(
        artifacts / "redundancy_clusters.csv", index=False, encoding="utf-8", lineterminator="\n"
    )
    pd.DataFrame(ledger_rows).to_csv(
        artifacts / "redundancy_ledger.csv", index=False, encoding="utf-8", lineterminator="\n"
    )
    component_panel.to_csv(
        artifacts / "risk_component_panel.csv", index=False, encoding="utf-8", lineterminator="\n"
    )
    evidence = {
        "schema_version": 1, "experiment_id": EXPERIMENT_ID, "decision": decision,
        "input_path_count": len(supported), "cluster_count": len(components),
        ...
        "reads_new_labels": False, "prototype_instantiated": False,
        "search_started": False, "candidate_created": False,
        "catalog_mutated": False, "platform_mutated": False, "pte_mutated": False,
    }
    _write(artifacts / "risk_component_review.json", evidence)
    (experiment / "03_execution.md").write_text(
        "# S008 EX40 执行记录\n\n"
        f"对{len(supported)}条60日回撤风险支持路径完成开发段相关性聚类，形成"
        f"{len(components)}个冗余簇和{len(component_panel)}项代表组件。没有读取新标签或实例化P04。\n",
        encoding="utf-8",
    )
    (experiment / "04_conclusion.md").write_text(
        "# S008 EX40 实验结论\n\n"
        f"机器裁决：`{decision}`。风险组件面板含{len(component_panel)}项低冗余信息，覆盖"
        f"{len(hypothesis_counts)}类经济假设和{len(family_counts)}个信息家族。"
        "组件只取得P04设计资格，尚未构成策略规则或候选证据。\n",
        encoding="utf-8",
    )
