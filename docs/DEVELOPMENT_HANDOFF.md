...
14. TDR的正式策略与研究候选共享同一个因果回放核心。每次回测显式指定不可变策略快照、
    实际标的、起止日和初始现金；同资产类型跨标的回测不修改策略快照或SM部署范围。
    生命周期资格不阻止研究回测，`PAPER_READY`只约束PTE部署。
15. `data/raw/`是受控研究池，`data/backtest/`独立维护。普通回测只读取已发布数据，不更新
    数据或判定数据污染；信号使用后复权数据，执行和估值使用不复权数据。TDR生成回放事实，
    SE负责数值化审计；研究数据边界由受控研究工作流负责。
