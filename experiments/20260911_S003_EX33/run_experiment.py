...
EXPERIMENT_ID = "20260911_S003_EX33"
...
def main() -> None:
    ...
    if any(
        ...
        raise ValueError("EX33 may only evaluate its frozen mechanism")
    ...
    name = protocol["research_target"]["mechanism"]
    ...
    (experiment / "03_execution.md").write_text(
        "# S003 EX33 执行\n\n"
        f"收益评价结果：`{status}`。{int(row['episodes'])}笔事件，基准均值"
        f"{row['baseline_mean_return']:.3%}、压力均值{row['stress_mean_return']:.3%}，压力盈亏比"
        f"{row['stress_profit_factor']:.2f}，正收益年份{int(row['positive_years'])}，最近252日事件"
        f"均值{row['recent_stress_mean_return']:.3%}，相对静态终值"
        f"{row['incremental_terminal_return_vs_static']:.2%}，账户最大回撤"
        f"{row['account_max_drawdown']:.2%}，卡玛{row['account_calmar']:.2f}。\n",
        encoding="utf-8",
    )
    conclusion = (
        "机制取得统计稳健性审计资格；审计必须覆盖S003全部已测试路径。"
        if qualified
