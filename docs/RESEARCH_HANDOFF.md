...
本文档记录截至 **2026-08-25 / 0825_EX04** 的可接力状态。新会话应先验证仓库当前状态，不能把本文中的分支或提交描述当成未经核验的实时事实。
...
| `scripts/run_experiment.py` | 按实验目录协议运行早期冠军挑战及0825_EX02—0825_EX04诊断 |
...
EX03—0825_EX04的运行器默认绑定各自实验目录。已有实验是冻结档案，不应为“复现”而直接覆盖运行。若要开展新实验，先创建新的 `MMDD_EXX` 目录并预注册协议，再显式传入 `--experiment-dir`。
...
EX08和0825_EX01拒绝“扩大同一搜索或重排同一Trial集合”；0825_EX02说明EX04附近是局部平台；0825_EX03发现退出侧是落后窗口的强线索；0825_EX04最终证明该线索由少数事件集中驱动，而不是离场信号普遍低质。20次分歧离场中11次具有材料性保护收益，现有信号日特征不能稳定识别5次错误离场。

下一步优先级：

1. 停止直接离场优化，不全局放宽阈值、不增加确认日、不延长最短持仓，也不把`joint_margin_block`直接改成门控；
2. 若继续研究，只能先解剖2021-06-16、2023-02-08、2021-07-06三个主导事件，寻找当日可知、尚未进入0825_EX04特征集的共同CZSC结构；不得从三天日期或事后路径直接构造规则；
3. 若路径解剖不能形成一个独立、可证伪且跨至少两个窗口的假设，结束离场方向，保留EX04现有离场；
4. 2025H2主要是入场问题，且其唯一离场事件为中性；若转向入场研究，必须作为全新单变量实验，不能混入离场修改；
...
5. 验证0824_EX01—0825_EX04实验清单，不依赖历史outputs。
...
重要边界：baseline_20260823是通用回测正式基线；EX04冻结策略只是EX05以后使用的研究基线。EX07完成608个SQLite Optuna Trial，EX08完成4096个纯内存Optuna Trial；0825_EX01复用EX08输出按三套规则冻结8个唯一候选并统一补测2026，三轮挑战均FAIL。0825_EX02—0825_EX04只用2020—2025完成EX04机制、交易路径及离场信号诊断，均为COMPLETE、不判PASS/FAIL、不访问2026；EX04离场诊断分类为path_concentrated_exit_failure，多数离场有效，少数错误离场主导损失，当前禁止直接优化。研究事实只认Git跟踪的experiments档案；outputs只用于当前设备新运行的普通回测。2026约束按单次实验执行：策略冻结前不可访问，冻结后一次验收，不得把验收结果反馈给同轮参数。
