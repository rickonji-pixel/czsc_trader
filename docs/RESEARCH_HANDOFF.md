...
- 2020—2025固定为研究集，持续同步到`data/raw`的2026行情用于补全项目内测试集；
- 项目的研究边界就是样本内，暂不考虑实盘反馈，也不宣称独立样本外有效；
- `0901_EX20`候选143已晋升为当前冠军；新研究仍须预注册目标、候选空间、数据边界和验收规则。

此前各实验对2026访问时点的限制仍是历史协议事实，不做事后改写。当前项目统一把2026称为持续补全的测试集；测试结果可以参与样本内冠军判断，但不能被表述为实盘反馈或独立样本外证明。
...
0901_EX21固定EX20全部6个合格候选，对2026Q1、H1和M1—M8的最大回撤、卡玛、盈亏比逐格按胜1、平0.5、负0计分。125、143、275、293均为8.0分并列第一，418为4.5分，400为2.5分；原活动基线自比较基准分为4.5。结合143在2021—2025研究集maximin排名第一，用户确认将143晋升为样本内冠军。
...
实际默认版本由`configs/rule_baselines/registry.json`的`latest`决定。`baseline_20260901`与`experiments/0901_EX20/artifacts/frozen_challenger.json`语义内容一致，是候选143的12因子条件权重四层策略：使用T-1及以前收盘价计算ER60，以冻结阈值`0.12654614652711713`划分trend/range，再选择对应权重；历史不足时使用`baseline_20260826`固定权重。它允许用于所有已验证标的的普通回测，但研究证据只来自`588080.SH`。
...
| 名称 | 用途 | 当前身份 |
|---|---|---|
| `baseline_20260901` | 默认普通回测、588080持续测试 | 通用活动基线，来源为0901_EX20候选143；跨标的可用不等于已证明通用有效 |
| `baseline_20260826` | 显式历史复现、候选143父基线 | 归档四层基线，来源为0824_EX04 |
...
它在同一连续窗口、费用和执行规则下同时提高收益、降低最大回撤；EX15随后对冻结规则执行2026回看，因截至8月28日没有出现新增事件，两套策略路径和指标完全相同，未获得晋升证据。该文件未写入基线注册表，普通回测现已改用`baseline_20260901`。
...
```text
CZSC因子（包含类别归一化） → ER60市场状态 → 条件权重 → 加权计分 → 入场/离场阈值
```

原来的“映射层”没有作为独立可优化层存在。类别到 `-1/0/+1` 或状态指示值的转换属于因子定义，避免同一语义在“因子”和“映射”两处重复表达。

活动基线仍使用EX04的12个因子、阈值和状态机，只把单一固定权重扩展为trend/range两套冻结权重。ER60严格滞后一日，因此当日目标只使用当日及以前的已完成信息，交易仍在下一交易日开盘执行。
...
研究链条可以概括为：0824_EX01—EX04建立并冻结EX04；0824_EX05—EX08扩大因子和搜索规模但未产生更优方案；0825_EX01—0826_EX01完成机制、路径、离场、新表示和边界诊断；0829_EX01—0830_EX08证明冻结冠军路径后的简单仓位覆盖层不能同时保持收益并改善回撤；0901_EX01—EX04暴露早期候选的稳定性不足；0901_EX05—EX14系统覆盖4,747个CZSC结构候选；EX15—EX18终止计分分档路线；EX19暴露无授权年度门槛，EX20纠错后冻结候选143，EX21再把6个合格候选统一放入三窗口评分。结合143的研究集第一名和测试集8.0分并列第一，用户确认将其晋升为`baseline_20260901`。
...
期望`origin`为SSH地址，活动基线为`baseline_20260901`，588080数据与全部实验档案验证为PASS。若任一事实不同，应以当前Git内容和命令输出为准，先查明差异，不得沿用本文快照继续执行。
...
未指定`--baseline`时，所有标的默认使用`baseline_20260901`。窗口文件只定义日期，不绑定标的、不包含收益目标；`--window`只运行指定窗口。必须读取命令返回的`artifacts.output_dir`，不得假设新设备存在任何历史输出目录。
...
7. 2020—2025固定为研究集；2026作为持续补全的项目内测试集。新增行情进入测试前必须先更新manifest并通过三频校验。
...
EX20/EX21已经产生并晋升候选143，证明regime条件权重是当前有效突破。0901_EX01重新检查CZSC信息源：`cxt_bi_zdf_V230601::向上`在锁定验证段的未来20日最大回撤差平均改善1.51个百分点，但收益方向不稳定；`cxt_second_bs_V230320::二卖`的验证均值优势受少数大涨样本驱动。后续研究应以新冠军为比较基准，而不是继续围绕已归档的固定权重基线优化。

下一步优先级：

1. `baseline_20260901`保持冻结，作为后续研究和普通回测的当前冠军；
...
7. 持续把2026交易数据同步到`data/raw`并补全测试集；固定Q1、H1，FULL窗口终点跟随最新已验证交易日；
8. `baseline_20260901`允许所有已验证标的默认用于普通回测，但其研究证据只来自588080；暂不把实盘反馈纳入研究。
...
3. .\.venv\Scripts\czsc-trader.exe baseline validate --version baseline_20260901 --symbol 588080.SH
4. .\.venv\Scripts\czsc-trader.exe archive validate --all
5. .\.venv\Scripts\python.exe -m pytest -q

重要边界：当前身份只认configs/rule_baselines/registry.json；baseline_20260901是通用活动正式基线，来源为EX20候选143，但研究证据只来自588080；baseline_20260826和baseline_20260823只用于显式历史复现。研究事实只认Git跟踪的实验档案；引用某轮精确事实前必须完整阅读该轮档案。outputs只用于当前设备新运行的普通回测，隔离回放产物也不是研究证据。2020—2025是研究集，持续补全的2026行情是项目内测试集；暂不考虑实盘反馈。
