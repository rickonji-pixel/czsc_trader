...
Trader负责计算限价、费用空间、目标仓位和100份整数倍委托数量。普通调仓返回
...
模拟盘买入使用DAY限价单，目标清仓的卖出使用DAY市价单。同一决策明确拒单后，只能在
...
用旧库存伪造一次完整轮换。进程重启不会丢失上述依赖关系。

排障时可单独补做某个策略版本的附加数据发布；普通策略会明确返回“不需要支持数据”：

```powershell
.\.venv\Scripts\czsc-trader.exe data prepare-strategy-support `
  --strategy S003 --strategy-version v1 --through 2026-09-11 `
  --data-dir state\paper_trading\data
```
