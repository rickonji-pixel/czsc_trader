...
EXPERIMENT_ID = "20260923_S008_EX22"
...
def _managed_directory(*, prefix: str, dir: Path):
    ...
    path = (root / "S008_EX22_runtime").resolve()
    if path.parent != root or path.name != "S008_EX22_runtime":
        raise ValueError("unsafe EX22 temporary path")
    if path.exists():
        shutil.rmtree(path)
...
def main() -> None:
    ...
    predecessor = repo / "experiments/S008/20260923_S008_EX21"
    ...
    paths = {
        "ex21_manifest_sha256": predecessor / "experiment_manifest.json",
    ...
    for name in ("03_execution.md", "04_conclusion.md"):
        ...
        path.write_text(
            path.read_text(encoding="utf-8").replace("# S008 EX20", "# S008 EX22"),
            encoding="utf-8",
        )
