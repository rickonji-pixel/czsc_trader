...
本文档记录截至 **2026-08-30 / 0830_EX02** 的可接力状态。新会话应先验证仓库当前状态，不能把本文中的分支或提交描述当成未经核验的实时事实。
...
测试集覆盖统一CLI端到端功能以及分数仓位状态机、分数仓位资金账本和因果审计，包含行情校验、基线、普通回测、实验档案、冻结保护和隔离回放。
...
EX08和0825_EX01拒绝“扩大同一搜索或重排同一Trial集合”；0825_EX02说明`0824_EX04`附近是局部平台；0825_EX03—0825_EX06逐步证明局部入退场病灶缺少稳定、低污染的新表示；0826_EX01未找到跨年稳定的简单偏离边界；0829_EX01的入场定仓在2026退化为冠军路径；0829_EX02的动态仓位产生了真实差异，但可见半年仅4/8胜出、平均收益增量为负，且原三个2026窗口只胜一个；0830_EX02的27组下行风险仓位均降低2021—2025收益，只有5组改善最大回撤，无法满足“收益不下降且回撤降低”的目标，因而没有访问2026。

下一步优先级：

1. `baseline_20260826`保持冻结，不再根据2020—2026已见历史修改12因子、权重、阈值或状态机；
2. `baseline_20260823`只用于显式历史复现，不再参与当前冠军判断；
3. 不依据0829_EX01、0829_EX02或0830_EX02结果缩小阈值步长、改变半仓比例、扩大下行风险参数网格或放宽收益约束；三类简单仓位路径均未产生可晋升方案，继续研究应先提出不同且有金融机制依据的新信息或架构；
...
3. 完整阅读0824_EX04、0826_EX01、0829_EX01、0829_EX02、0830_EX01和0830_EX02的目标、设计、执行、结论及manifest，确认当前基线来源、停止回溯修补的依据、仓位实验结果以及ERROR到逐字重试FAIL的链路。
