...
EXPERIMENT_ID = "20260923_S008_EX16"
...
def main() -> None:
    ...
    predecessor_manifest = repo / "experiments/S008/20260923_S008_EX15/experiment_manifest.json"
    ...
    if _sha256(predecessor_manifest) != overlay["predecessor_manifest_sha256"]:
        raise ValueError("EX15 predecessor manifest differs from frozen identity")
    ...
    def inherited_read(path: Path) -> dict[str, object]:
        ...
        protocol["sources"].update(overlay["updated_sources"])
        return protocol
    ...
    for name in ("03_execution.md", "04_conclusion.md"):
        ...
        path.write_text(
            path.read_text(encoding="utf-8").replace("# S008 EX14", "# S008 EX16"),
            encoding="utf-8",
        )
