...
11. `realized_vol5_to20`：5日简单收益样本标准差（`ddof=1`）/20日同口径样本标准差；
12. `downside_semivol5_to20`：最近5个简单收益中`min(ret,0)`在全部5个观测上的均方根/最近20个收益同口径均方根；分母为0时缺失；
13. `negative_day_share5`：最近5日负收益日数/5；
14. `max_drawdown5_to_atr20`：最近5个收盘观测中任意先高后低路径的最大回撤绝对值除以当日`ATR20/C`；不足5个收盘时缺失；
...
22. `return_volume_corr10`：最近10个简单收益与同期`log1p(V)`的样本Pearson相关；必须有10对有限值且两列方差均非0；
...
25. `intraday_realized_vol_to20`：T日30分钟收益平方和平方根/此前20个完整交易日同口径中位数；首根收益为开盘到收盘，其余为相邻收盘收益，当前T日不得进入历史中位数；
26. `intraday_close_location`：T日收盘在当日30分钟最高价与最低价区间中的位置；
27. `last4_30m_return`：T日最后4根完整30分钟K线的首开至末收收益；
28. `weekly_close_to_ma10`：最近完整周线收盘/包含该周在内的最近10根完整周线收盘均值-1；若T等于周线时间戳，该周在T收盘后视为已完成，否则只允许使用上一根完整周线。

不得增加交互项、AND/OR组合、替代公式或临时描述符。实现若发现某公式无法按上述口径唯一计算，应在正式运行前回到设计评审，不得自行改义。

## 4. 因果历史分位箱

每项日线或日内描述符在事件T的值，只与该描述符在T之前252个交易日的历史值比较，最少要求120个有限历史值；分位点使用线性插值，按固定的20%、40%、60%、80%切分为`Q1`—`Q5`，边界值按右闭规则进入较低分位箱。周线描述符先在每根完整周线上计算，再使用当前周线之前26根完整周线的描述符值，最少要求20个有限值；计算出原始值和分位箱后才向后对齐到事件T，重复对齐的日线日期不得被当成独立周线历史观测。
