...
本文档记录截至 **2026-08-29 / 0829_EX01** 的可接力状态。新会话应先验证仓库当前状态，不能把本文中的分支或提交描述当成未经核验的实时事实。
...
研究链条可以概括为：0824_EX01—EX04建立并冻结EX04；0824_EX05—EX08扩大因子和搜索规模但未产生更优方案；0825_EX01对EX08候选补测仍全部失败；0825_EX02—EX06完成机制、路径、离场和新表示诊断；0826_EX01未找到跨年稳定的偏离边界；0829_EX01首次把动作扩展为入场半仓/满仓，但冻结方案在三个原2026窗口没有形成任何路径差异，按原规则FAIL。
...
EX08和0825_EX01拒绝“扩大同一搜索或重排同一Trial集合”；0825_EX02说明`0824_EX04`附近是局部平台；0825_EX03—0825_EX06逐步证明局部入退场病灶缺少稳定、低污染的新表示；0826_EX01未找到跨年稳定的简单偏离边界；0829_EX01又表明首个入场定仓方案在选择期仅3/8胜出，并在三个原2026窗口完全退化为冠军路径。

下一步优先级：

1. `baseline_20260826`保持冻结，不再根据2020—2026已见历史修改12因子、权重、阈值或状态机；
2. `baseline_20260823`只用于显式历史复现，不再参与当前冠军判断；
3. 不依据0829_EX01结果缩小阈值步长、补加阈值或把持有期动态调仓混入同一轮；若继续仓位研究，必须另立可证伪预注册方案；
...
3. 完整阅读0824_EX04、0826_EX01和0829_EX01的目标、设计、执行、结论及manifest，确认当前基线来源、停止回溯修补的依据和首轮仓位实验结果。
