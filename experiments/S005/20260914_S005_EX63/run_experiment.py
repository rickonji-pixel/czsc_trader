...
EXPERIMENT_ID = "20260914_S005_EX63"
...
def main() -> None:
    ...
    if any(protocol.get(key) for key in ("candidate_generation", "promotion_allowed", "mutates_strategy_manager", "mutates_pte")):
        raise ValueError("EX63 cannot create, promote, or deploy a candidate")
    ...
    if selection.get("selected_mechanism") != mechanism or selection.get("decision") != "PROCEED_TO_FIXED_SUPPLY_ABSORPTION_RETURN_TEST":
    ...
    for event in events.itertuples(index=False):
        ...
        rows.append({
            ...
            "exit_date": exit_date,
            "score": float(event.supply_absorption_score),
    ...
    decision = "PROCEED_TO_SUPPLY_ABSORPTION_ATTRIBUTION" if passed else "STOP_SUPPLY_ABSORPTION_ON_RETURN"
    evidence = {
        "schema_version": 1,
