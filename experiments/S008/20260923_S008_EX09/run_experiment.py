...
EXPERIMENT_ID = "20260923_S008_EX09"
...
def _safe_haven_shock_position(
    close: pd.Series,
    shock_horizon: int,
    ...
    shock_return = log_close - log_close.shift(shock_horizon)
    shock_volatility = daily_log_return.rolling(60).std(ddof=0) * np.sqrt(shock_horizon)
    shock_score = shock_return / shock_volatility.replace(0.0, np.nan)
    short_volatility = daily_log_return.rolling(volatility_window).std(ddof=0)
    volatility_baseline = short_volatility.shift(1).rolling(
        volatility_baseline_window
    ).median()
    volatility_expanding = (
        short_volatility > volatility_baseline * volatility_expansion_ratio
    )
    exit_average = close.astype(float).ewm(span=exit_ema_span, adjust=False).mean()
    entry_event = (shock_score > shock_threshold) & volatility_expanding
    exit_event = close < exit_average
...
def main() -> None:
    ...
    configurations = [
        {
            "name": f"shock_horizon={horizon};shock_score={score:g}",
            "coordinates": (horizon_index, score_index),
            "shock_horizon": int(horizon),
            "shock_score": float(score),
        }
        for horizon_index, horizon in enumerate(grid["shock_horizon"])
        for score_index, score in enumerate(grid["shock_score"])
    ]
    ...
    for symbol in protocol["symbols"]:
        ...
        for configuration in configurations:
            positions[configuration["name"]] = _safe_haven_shock_position(
                close,
                configuration["shock_horizon"],
                configuration["shock_score"],
                int(grid["volatility_window"]),
                int(grid["volatility_baseline_window"]),
                float(grid["volatility_expansion_ratio"]),
                int(grid["exit_ema_span"]),
            )
        positions["always_long"] = pd.Series(1.0, index=close.index)
        for family, names in (
            ("BUY_HOLD", ["always_long"]),
            ("SAFE_HAVEN_SHOCK", [item["name"] for item in configurations]),
        ):
            for name in names:
    ...
    for configuration in configurations:
        ...
        for symbol in protocol["symbols"]:
            ...
            full10 = lookup[(symbol, "SAFE_HAVEN_SHOCK", name, 10, "FULL")]
            full30 = lookup[(symbol, "SAFE_HAVEN_SHOCK", name, 30, "FULL")]
            positive_periods = all(
                lookup[(symbol, "SAFE_HAVEN_SHOCK", name, 10, period)][
                    "annualized_return"
                ]
                > 0
    ...
    (experiment / "03_execution.md").write_text(
        "# S008 EX09 执行记录\n\n"
        f"按冻结协议在两只ETF上评价{len(configurations)}个避险冲击配置、两档成本和"
