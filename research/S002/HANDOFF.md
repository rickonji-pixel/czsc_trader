...
- 当前开发截止：`2026-09-08`；
- 最近实验：`experiments/S002/20260910_S002_EX16`。

## 研究目标与固定边界

S002批次目标是为510500.SH构建一套金融逻辑可解释、低成本且可由TDR与PTE一致复现的
独立择时策略。账户只持有510500.SH或现金，不使用杠杆和做空；首个候选以同标的、同资金、
同窗口和同执行口径的BuyHold为明确对手，并依次通过候选登记、正式PK、完整体检和人工冻结。
具体数值门槛、成本口径及风险保留意见以EX12—EX16冻结证据为准，不继承其他`SXX`目标。
