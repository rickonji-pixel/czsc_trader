...
EXPERIMENT_ID = "20260913_S005_EX33"
...
def main() -> None:
    ...
    expected = {
        ...
        source / "artifacts/nav_dislocation_feature_ledger.csv.gz": source_spec["ledger_sha256"],
    ...
    ledger = pd.read_csv(source / "artifacts/nav_dislocation_feature_ledger.csv.gz", parse_dates=["trade_date"]).set_index("trade_date")
    ...
    decision = "PROCEED_TO_MECHANISM_ATTRIBUTION" if all(checks.values()) else "STOP_ETF_DISCOUNT_REVERSION_ON_RETURN"
    evidence = {
        "schema_version": 1,
    ...
    (experiment / "03_execution.md").write_text(
        "# S005 EX33 执行\n\n"
        f"状态：`COMPLETE`。按固定T+1日09:36入场、第三个持有日14:56退出口径，"
