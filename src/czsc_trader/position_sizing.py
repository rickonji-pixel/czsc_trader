...
def positions_from_dynamic_score_zones(
    scores: pd.Series,
    enter: float,
    exit_: float,
    state_rule: Rule,
    *,
    ...
    """Enter full, then resize between full/partial using the original score zones."""
    enter = float(enter)
    exit_ = float(exit_)
    partial_position = float(partial_position)
    if not np.isfinite([enter, exit_, partial_position]).all():
        raise ValueError("dynamic position-sizing inputs must be finite")
    if not exit_ < enter:
        raise ValueError("dynamic position-sizing thresholds must satisfy exit < enter")
    if not 0.0 < partial_position < 1.0:
        raise ValueError("partial_position must be strictly between 0 and 1")
    if state_rule.entry_gate != "none":
        raise ValueError("dynamic sizing requires baseline entry_gate none")
    values = scores.astype(float)
    if values.isna().any() or not np.isfinite(values).all():
        raise ValueError("dynamic position-sizing scores must be finite")
    if not values.index.is_monotonic_increasing or values.index.has_duplicates:
        raise ValueError("dynamic position-sizing score index must be unique and increasing")
    ...
    for score in values:
        if position == 0.0:
            ...
            if eligible and score <= exit_:
                ...
                exit_confirmations = 0
                if eligible:
                    position = 1.0 if score >= enter else partial_position
            if position > 0.0:
                holding_days += 1
        output.append(position)
    return pd.Series(output, index=values.index, name="target_position", dtype=float)
...
def build_dynamic_sizing_events(
    target_position: pd.Series,
    scores: pd.Series,
    enter: float,
    exit_: float,
) -> pd.DataFrame:
    """Describe every full/partial/cash transition for causal audit."""
    if not target_position.index.equals(scores.index):
        raise ValueError("dynamic-sizing target and score indices must match")
    target = target_position.astype(float)
    if target.isna().any() or not target.isin([0.0, 0.5, 1.0]).all():
        raise ValueError("dynamic-sizing target must contain only 0, 0.5, or 1")
    previous = target.shift(1, fill_value=0.0)
    rows: list[dict[str, object]] = []
    for signal_date in target.index[target.ne(previous)]:
        before = float(previous.loc[signal_date])
        ...
        if before == 0.0 and after == 1.0:
            event_type = "Entry"
            reason = f"score {float(scores.loc[signal_date]):.6f} >= enter {float(enter):.6f}"
        elif before == 1.0 and after == 0.5:
            event_type = "Reduce"
            reason = (
                f"exit {float(exit_):.6f} < score {float(scores.loc[signal_date]):.6f} "
                f"< enter {float(enter):.6f}"
            )
        elif before == 0.5 and after == 1.0:
            event_type = "Increase"
            reason = f"score {float(scores.loc[signal_date]):.6f} >= enter {float(enter):.6f}"
        elif before > 0.0 and after == 0.0:
            event_type = "Exit"
            reason = f"score {float(scores.loc[signal_date]):.6f} <= exit {float(exit_):.6f}"
        else:
            raise ValueError(f"unsupported dynamic position transition {before} -> {after}")
        rows.append(
            {
                "event_id": f"DynamicSizing:{pd.Timestamp(signal_date):%Y%m%d}:{event_type}",
                "signal_date": pd.Timestamp(signal_date),
                "event_type": event_type,
                "factor_score": float(scores.loc[signal_date]),
                "enter_threshold": float(enter),
                "full_threshold": float(enter),
                "exit_threshold": float(exit_),
                "before_position": before,
                "after_position": after,
                "reason": reason,
