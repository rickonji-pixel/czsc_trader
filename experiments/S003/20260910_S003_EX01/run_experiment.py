...
EXPERIMENT_ID = "20260910_S003_EX01"
...
def main() -> None:
    ...
    if any(bool(protocol.get(key)) for key in forbidden):
        raise ValueError("data feasibility experiment may not mutate research lifecycle state")
    ...
    daily, daily_metadata = fetch_etf_ohlcv(
        str(target["symbol"]), start, end, "daily", env_file=env_file
    )
    ...
    for period in ("15m", "5m", "1m"):
        frame, metadata = fetch_etf_ohlcv(
            str(target["symbol"]), start, end, period, env_file=env_file
        )
        ...
        frequency_results[period] = {
            ...
            "first_session_close": min(session["session_times"]),
            "last_session_close": max(session["session_times"]),
    ...
    (experiment / "04_conclusion.md").write_text(
        ...
        "三个周期与日线的OHLCV聚合一致，可以进入完整历史数据发布与S003信号普查。\n\n"
        "本轮只证明数据接口和对账契约可用，不证明任何分钟信号或策略有效。下一步应先发布"
        "完整开发池分钟数据，再冻结日线regime定义并开展15m信号普查。\n",
        encoding="utf-8",
    )
