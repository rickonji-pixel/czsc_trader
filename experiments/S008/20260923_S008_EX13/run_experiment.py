...
EXPERIMENT_ID = "20260923_S008_EX13"
...
def main() -> None:
    ...
    source_paths = {
        ...
        "ex12_manifest_sha256": repo
        / "experiments/S008/20260923_S008_EX12/experiment_manifest.json",
    ...
    for key, path in source_paths.items():
        if _sha256(path) != str(sources[key]):
    ...
    for specification in protocol["datasets"]:
        ...
        row: dict[str, object] = {
            ...
            "symbol": specification["symbol"] or "",
        ...
        duplicate_primary_keys = int(frame.duplicated(["Date"]).sum())
        expected_start = pd.Timestamp(str(specification["expected_start"]))
        expected_cutoff = pd.Timestamp(str(specification["expected_cutoff"]))
        actual_start = pd.Timestamp(identity.data_start)
        actual_cutoff = pd.Timestamp(identity.data_cutoff)
        data_contract_pass = bool(
            len(frame) >= int(specification["minimum_rows"])
            and actual_start.normalize() == expected_start
            and actual_cutoff.normalize() == expected_cutoff
    ...
    (experiment / "03_execution.md").write_text(
        "# S008 EX13 执行记录\n\n"
        f"通过DFLS公开门面请求{len(rows)}项数据；READY={summary['ready_count']}，"
        f"数据合同通过={summary['data_contract_pass_count']}，因果元数据通过="
        f"{summary['causality_metadata_pass_count']}。只归档身份和检查摘要，没有保存供应商"
        "原始数据、读取目标收益或封存验证池。\n",
        encoding="utf-8",
    )
    (experiment / "04_conclusion.md").write_text(
        "# S008 EX13 结论\n\n"
        f"裁决：`{decision}`。发布失败={publication_failures or '无'}；数据合同失败="
        f"{data_failures or '无'}；因果元数据失败={causality_failures or '无'}。"
        "只有全部通过时，下一步才允许物化截至2024-12-31的因果特征面板；本实验不形成"
        "任何Alpha或候选证据。\n",
        encoding="utf-8",
    )
