...
def calculate_preference_history(
    ...
    # Preserve every foreign-market observation and select the latest source
    # strictly before each China session, matching the EX53 information audit.
    xau_mid, xau_source_date = _strict_prior(_mid_close(xau, _XAU), index)
    xag_mid, xag_source_date = _strict_prior(_mid_close(xag, _XAG), index)
...
class S008PreciousMetalPreference(StrategyImplementation):
    ...
    def __init__(self, candidate: StrategyCandidate) -> None:
        ...
        self._definition = RuntimeDefinition(
            ...
            history=HistoryPolicy("CANONICAL_REPLAY", "2013-07-29", "2013-01-02"),
        )
