...
本文档记录截至 **2026-09-01 / 0901_EX01** 的可接力状态。0901_EX01在独立研究分支完成，尚未合并或推送；新会话应先验证仓库当前状态，不能把本文中的分支、提交或日期描述当成未经核验的实时事实。
...
研究链条可以概括为：0824_EX01—EX04建立并冻结EX04；0824_EX05—EX08扩大因子和搜索规模但未产生更优方案；0825_EX01对EX08候选补测仍全部失败；0825_EX02—EX06完成机制、路径、离场和新表示诊断；0826_EX01未找到跨年稳定的偏离边界；0829_EX01—0830_EX08证明冻结冠军路径后的简单仓位覆盖层不能同时保持收益并改善回撤；0901_EX01因此回到CZSC信息源，找到一个暂定风险状态和一个脆弱事件因子，但尚未形成或验证统一仓位策略。
...
既有搜索和仓位覆盖层路线均未产生可晋升策略。0901_EX01重新检查CZSC信息源：`cxt_bi_zdf_V230601::向上`在锁定验证段的未来20日最大回撤差平均改善1.51个百分点，但收益方向不稳定；`cxt_second_bs_V230320::二卖`的验证均值优势受少数大涨样本驱动。两者都不是策略胜出证据。
...
4. 下一轮CZSC因子研究优先保留原始信号的多值结构，预注册`cxt_bi_zdf`的“方向×力度层级”，不要继续只取第一值；
5. `二卖`只作为脆弱候选保留，除非新的事件稳健性协议通过，不得直接赋予正向权重；
6. 新因子通过稳定性诊断后另开统一多档仓位实验，并继续以收益不低于冠军、最大回撤严格改善为硬门槛；
7. 从2026-08-28之后继续积累前向表现；截至该日的数据不得重新表述为独立留出；
...
3. 完整阅读0824_EX04、0826_EX01、0829_EX01、0829_EX02、0830_EX01—EX08和0901_EX01的目标、设计、执行、结论及manifest。
4. 因子问题优先读0901_EX01，再补读0824_EX06—EX08和0825_EX01；归因或信号问题再补读0825_EX02—EX06。
