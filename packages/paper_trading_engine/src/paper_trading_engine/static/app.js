...
function renderSystemEvents(s){
  ...
  const history=(s.events||[]).length?(s.events||[]).map(item=>`<div class="event"><div class="event-title"><strong>${esc(systemEventLabel(item.event_type))}</strong><time>${esc(item.created_at)}</time></div>${renderEventDetails(item.payload)}</div>`).join(''):'<div class="empty">暂无系统事件</div>';
