...
本项目只读取 `data/raw` 中的 588080.SH K线，通过 CZSC 1.0.1 生成多周期因子，从预先声明的候选空间选择一个固定规则，并由 vectorbt 1.1.0 在下一交易日开盘执行和验证。最终仓位只能由 CZSC 因子状态机产生；收益、基准、日期或组合净值不得覆盖仓位。候选选择允许使用已发生的2026年结果，因此报告明确标记为样本内优化，而非样本外验证。
...
副图展示结构、趋势、量价位置与综合因子分数。五笔/七笔背驰同时进入结构因子。
