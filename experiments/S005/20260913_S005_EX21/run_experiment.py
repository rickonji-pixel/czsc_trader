...
EXPERIMENT_ID = "20260913_S005_EX21"
...
def main() -> None:
    ...
    expected = {
        ...
        source / "artifacts/constituent_feature_ledger.csv.gz": source_spec["ledger_sha256"],
    ...
    for mechanism in protocol["mechanisms"]:
        if mechanism not in selection["density_passing_paths"]:
            raise ValueError(f"mechanism did not pass frozen density gate: {mechanism}")

    ledger = pd.read_csv(
        source / "artifacts/constituent_feature_ledger.csv.gz", parse_dates=["dt"]
    ).set_index("dt")
    ...
    for mechanism in protocol["mechanisms"]:
        ...
        annual_frames.append(annual.reset_index())

    passing = [str(row["mechanism"]) for row in evidence_rows if row["passed"]]
    decision = (
        "PROCEED_TO_MECHANISM_ATTRIBUTION"
        if passing
        else "STOP_CONSTITUENT_MECHANISMS_ON_RETURN"
    )
    evidence = {
        "schema_version": 1,
