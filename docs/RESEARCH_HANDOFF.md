...
本文档记录截至 **2026-09-01 / 0901_EX19** 的可接力状态。0901_EX05—EX18已纳入`master`；0901_EX19在独立研究分支完成两档regime条件权重挑战，但尚未合并或推送。新会话仍应先验证仓库当前状态，不能把本文中的分支、提交或日期描述当成未经核验的实时事实。
...
0901_EX19已经检验活动基线固定线性权重在因果ER60两档市场状态下的条件化改进空间。625项中6项在2021—2025连续窗口同时改善最大回撤、卡玛和盈亏比，但年度最大回撤与卡玛同时改善最多只有2/5，未达到预注册3/5门槛；实验FAIL，未冻结挑战者、未读取2026。该结果只覆盖ER60研究中位数分界、固定三因子组与五档乘数网格，不外推为所有regime权重无效。
