...
EXPERIMENT_ID = "20260915_S007_EX27"
...
def _audit_platform(
    ...
    def normalized_params(frame: pd.DataFrame) -> pd.DataFrame:
        ...
        if (
            ...
            raise ValueError("normalized parameters are non-finite or outside EX27 bounds")
        return normalized
...
def main() -> None:
    ...
    if protocol.get("experiment_id") != EXPERIMENT_ID or not protocol.get("reads_new_returns"):
        raise ValueError("EX27 protocol identity or return declaration differs")
    if any(protocol.get(key) for key in ("candidate_generation", "promotion_allowed", "mutates_strategy_manager", "mutates_pte")):
        raise ValueError("EX27 cannot create, promote, or deploy a candidate")

    sources = protocol["sources"]
    ex26 = repo / str(sources["ex26_archive"])
    ...
    validate_experiment_archive(ex26)
    ...
    frozen = {
        ex26 / "experiment_manifest.json": sources["ex26_manifest_sha256"],
        ex26 / "artifacts/platform_evidence.json": sources["ex26_platform_evidence_sha256"],
        ex26 / "artifacts/search_trial_ledger.csv.gz": sources["ex26_search_ledger_sha256"],
        ex26 / "artifacts/parameter_components.csv": sources["ex26_parameter_components_sha256"],
    ...
    for path, expected in frozen.items():
        if _sha256(path) != expected:
            raise ValueError(f"frozen EX27 source differs: {path}")
    ...
    if search["storage"] != "IN_MEMORY" or search["method"] != "nsga2":
        raise ValueError("EX27 search engine differs from protocol")
    ...
    spec = SearchSpec(
        study_name="S007-EX27-DECOUPLED-ENTRY-BOUNDARY",
        method="nsga2",
        parameters=(
    ...
    current_components = pd.read_csv(artifacts / "parameter_components.csv")
    current_largest = current_components.sort_values(
        ["unique_behaviors", "parameter_points"], ascending=False
    ).iloc[0]
    current_trial_ids = json.loads(str(current_largest["trial_ids"]))
    current_platform = enriched.set_index("trial_id").loc[current_trial_ids].copy()
    current_entry_quantiles = current_platform["params"].map(
        lambda value: float(json.loads(value)["entry_quantile"])
    )
    old_entry_upper = 0.80
    expanded_mask = current_entry_quantiles > old_entry_upper
    feasible_entry_quantiles = feasible["params"].map(
        lambda value: float(json.loads(value)["entry_quantile"])
    )
    pareto_entry_quantiles = pareto["params"].map(
        lambda value: float(json.loads(value)["entry_quantile"])
    )
    comparison_columns = [
        "objective.cagr",
        ...
        "objective.calmar",
        "metric.full.rolling_60_closed_trades_median",
        "metric.full.rolling_60_closed_trades_p10",
    ]

    def metric_medians(frame: pd.DataFrame) -> dict[str, float] | None:
        if frame.empty:
            return None
        medians = frame[comparison_columns].median()
        return {name: float(medians[name]) for name in comparison_columns}

    boundary_extension_diagnostic = {
        "schema_version": 1,
        "experiment_id": EXPERIMENT_ID,
        "diagnostic_role": "DESCRIPTIVE_NOT_ADDITIONAL_GATE",
        "old_entry_quantile_upper": old_entry_upper,
        "new_entry_quantile_upper": float(bounds["entry_quantile"][1]),
        "feasible_trials": int(len(feasible)),
        "feasible_trials_above_old_upper": int((feasible_entry_quantiles > old_entry_upper).sum()),
        "feasible_share_above_old_upper": float((feasible_entry_quantiles > old_entry_upper).mean()),
        "largest_platform_points": int(len(current_platform)),
        "largest_platform_points_above_old_upper": int(expanded_mask.sum()),
        "largest_platform_share_above_old_upper": float(expanded_mask.mean()),
        "largest_platform_entry_quantiles": {
            "minimum": float(current_entry_quantiles.min()),
            "q10": float(current_entry_quantiles.quantile(0.10)),
            "median": float(current_entry_quantiles.median()),
            "q90": float(current_entry_quantiles.quantile(0.90)),
            "maximum": float(current_entry_quantiles.max()),
        },
        "old_region_metric_medians": metric_medians(current_platform.loc[~expanded_mask]),
        "expanded_region_metric_medians": metric_medians(current_platform.loc[expanded_mask]),
        "pareto_trials": int(len(pareto)),
        "pareto_trials_above_old_upper": int((pareto_entry_quantiles > old_entry_upper).sum()),
    }
    _write(artifacts / "boundary_extension_diagnostic.json", boundary_extension_diagnostic)
    ex26_components = pd.read_csv(ex26 / "artifacts/parameter_components.csv")
    ex26_largest = ex26_components.sort_values(
        ["unique_behaviors", "parameter_points"], ascending=False
    ).iloc[0]
    ex26_trial_ids = json.loads(str(ex26_largest["trial_ids"]))
    ex26_ledger = pd.read_csv(ex26 / "artifacts/search_trial_ledger.csv.gz").set_index("trial_id")
    ex26_platform = ex26_ledger.loc[ex26_trial_ids]
    reference = {
        "cagr_median": float(ex26_platform["objective.cagr"].median()),
        "maximum_drawdown_median": float(ex26_platform["objective.max_drawdown"].median()),
        "calmar_median": float(ex26_platform["objective.calmar"].median()),
        "frequency_median_median": float(
            ex26_platform["metric.full.rolling_60_closed_trades_median"].median()
        ),
        "frequency_p10_median": float(
            ex26_platform["metric.full.rolling_60_closed_trades_p10"].median()
        ),
        "parameter_points": int(len(ex26_platform)),
        "unique_behaviors": int(ex26_platform["behavior_hash"].nunique()),
    }
    decoupled = {
    ...
    decision = (
        ...
        else "REVIEW_EXTENDED_BOUNDARY_REQUIRED"
        if platform_audit["boundary_dependent"]
        else "PREFER_EXTENDED_ENTRY_BOUNDARY_REVIEW"
        if all_not_worse
        else "RETAIN_EX26_BOUNDARY_REVIEW"
        if all_not_better
    ...
    evidence = {
        ...
        "boundary_extension_diagnostic": boundary_extension_diagnostic,
        "prototype_comparison": {
            "reference_ex26": reference,
            "expanded_ex27": decoupled,
            "expanded_minus_reference": comparison_deltas,
