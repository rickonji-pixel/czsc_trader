...
EXPERIMENT_ID = "20260913_S004_EX27"
...
def _build_features(panel: pd.DataFrame, daily: pd.DataFrame) -> pd.DataFrame:
    ...
    frame["advance"] = observed & frame["pct_chg"].gt(0)
    frame["observed_weight"] = frame["weight"].where(observed, 0.0)
    frame["weighted_return_numerator"] = (
        frame["weight"] * frame["pct_chg"]
    ).where(observed, 0.0)
    ...
    features["etf_volatility_20"] = features["etf_return_pct"].rolling(
        20, min_periods=20
    ).std(ddof=1)
    features["prior_volatility_median_120"] = features["etf_volatility_20"].shift(1).rolling(
        120, min_periods=120
    ).median()
...
def _attach_returns(events: pd.DataFrame, market, cost_per_side_bps: float) -> pd.DataFrame:
    ...
    output["stress_return"] = output["gross_return"] - 2 * cost_per_side_bps / 10_000.0
...
def _metrics(
    ...
    density = _density(trades, calendar, start)
...
def main() -> None:
    ...
    if protocol.get("candidate_generation") or protocol.get("promotion_allowed"):
        raise ValueError("EX27 may only evaluate the frozen mechanism")

    sources = protocol["sources"]
    primary_source = repo / "experiments" / "S004" / sources["primary_event_experiment"]
    replication_source = (
        repo / "experiments" / "S003" / sources["replication_panel_experiment"]
    )
    validate_experiment_archive(primary_source)
    validate_experiment_archive(replication_source)
    expected = {
        ...
        primary_source / "artifacts" / "breadth_lead_features.csv.gz": sources[
            "primary_features_sha256"
        ],
        replication_source / "experiment_manifest.json": sources[
            "replication_manifest_sha256"
        ],
        replication_source / "artifacts" / "lifecycle_aware_constituent_panel.csv.gz": sources[
            "replication_panel_sha256"
        ],
    ...
    replication_panel = pd.read_csv(
        replication_source / "artifacts" / "lifecycle_aware_constituent_panel.csv.gz"
    )
    replication_features = _build_features(replication_panel, replication_market.daily)
    ...
    summary = {
        ...
        "route_decision": "CONTINUE_MECHANISM_AUDIT" if passed else "STOP_BREADTH_LEAD_FAMILY",
    ...
    (experiment / "03_execution.md").write_text(
        "# S004 EX27 执行\n\n"
        f"固定规则收益评价：`{summary['status']}`。\n\n" + "\n".join(lines) + "\n\n"
        "本轮是第68次收益检验，没有改变阈值或执行时点。\n",
        encoding="utf-8",
    )
    conclusion = (
        "双标的全样本与低波动子样本均通过预注册门，可进入统计与竞争解释审计。"
        if passed
        else "至少一个预注册必要条件失败，停止成分领先机制，不创建S004-C002。"
    )
