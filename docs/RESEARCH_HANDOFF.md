...
本文档记录截至 **2026-08-25 / 0825_EX02** 的可接力状态。新会话应先验证仓库当前状态，不能把本文中的分支或提交描述当成未经核验的实时事实。
...
EX03—0825_EX02的运行器默认绑定各自实验目录。已有实验是冻结档案，不应为“复现”而直接覆盖运行。若要开展新实验，先创建新的 `MMDD_EXX` 目录并预注册协议，再显式传入 `--experiment-dir`。
...
EX08和0825_EX01已经拒绝“扩大同一搜索或重排同一Trial集合”。0825_EX02进一步说明EX04附近是局部平台，但没有跨年度稳定正向的单一组件或因子组；它主要在下跌和震荡状态降低无效暴露，在上涨状态损失机会。

下一步优先级：

1. 首选冻结外推：在其他已准备ETF或未来未见月份上，不调参复核EX04相对通用基线的防守型状态归因是否迁移；
2. 若归因能够迁移，再预注册新的状态表达或策略架构，检验能否保留下跌/震荡优势并减少上涨机会损失；
3. 保持四层因果执行和EX04公平研究基线，不再以局部权重、阈值微调或增加Trial作为单一变化；
4. 量价位置组虽历史中位贡献偏负，但2025显著正向，属于状态依赖，不得直接删除；
5. 新实验仍只用2020—2025选取和冻结，2026不得反馈同轮参数。
...
5. 验证0824_EX01—0825_EX02实验清单，不依赖历史outputs。
...
重要边界：baseline_20260823是通用回测正式基线；EX04冻结策略只是EX05以后使用的研究基线。EX07完成608个SQLite Optuna Trial，EX08完成4096个纯内存Optuna Trial；0825_EX01复用EX08输出按三套规则冻结8个唯一候选并统一补测2026，三轮挑战均FAIL。0825_EX02只用2020—2025完成EX04机制归因，结论COMPLETE，不判PASS/FAIL、不访问2026；EX04表现为状态依赖的防守型局部平台。研究事实只认Git跟踪的experiments档案；outputs只用于当前设备新运行的普通回测。2026约束按单次实验执行：策略冻结前不可访问，冻结后一次验收，不得把验收结果反馈给同轮参数。
