export function parseRoute(pathname) {
  ...
  if (pathname === '/channels/futu-simulate-us') return {page: 'channel', channel: 'futu_simulate_us'};
...
export const auditScopeLabel = (event,accounts=[]) => {
  ...
  if(event?.channel==='futu_simulate_us')return 'Futu模拟盘US渠道';
  return '历史记录 · 作用域未记录';
};
...
export const channelSlug = channel => channel==='futu_simulate_us'?'futu-simulate-us':'futu-simulate-cn';
...
async function refreshSystem(){const version=document.querySelector('#releaseVersion');try{const s=await getJson('/api/system/status');const fingerprint=snapshotFingerprint(s);const changed=fingerprint!==state.systemFingerprint;state.system=s;state.systemFingerprint=fingerprint;const market=s.channel_id==='futu_simulate_us'?'US':'CN';const nav=document.querySelector('[data-nav="channel"]');nav.href=`/channels/${channelSlug(s.channel_id)}`;nav.textContent=`Futu模拟盘${market}`;const count=systemAlertCount(s);const release=releaseVersionLabel(s);version.textContent=release;version.classList.toggle('warning',release==='版本未知');const status=document.querySelector('#systemStatus');status.textContent=`PTE ${statusLabel(s.runtime)} · Futu模拟盘${market} ${statusLabel(s.futu_connection)} · 全局告警 ${count} · 查看系统事件`;status.classList.toggle('warning',count>0);if(changed&&document.querySelector('#systemEventsDialog')?.open)renderSystemEvents(s);}catch(error){version.textContent='版本未知';version.classList.add('warning');document.querySelector('#systemStatus').textContent=`系统状态不可用 · ${error.message}`;}}
...
function renderChannel(s){const detailsOpen=document.querySelector('#channelReconciliationDetails')?.open||false;const slug=channelSlug(s.scope?.channel);const html=channelMarkup(s,detailsOpen);document.querySelector('#app').innerHTML=slug==='futu-simulate-us'?html.replaceAll('模拟盘 CN','模拟盘 US'):html;document.querySelector('#channelSwitch').onclick=async()=>{try{await post(`/api/channels/${slug}/${s.paused?'resume':'pause'}`);toast(s.paused?'渠道已恢复':'渠道已暂停');await loadRoute({showLoading:false});}catch(e){toast(e.message);}};document.querySelectorAll('#channelOrders tbody tr').forEach((tr,i)=>{const o=s.orders?.[i];if(o&&['SUBMITTED','FILLED_PART'].includes(o.status))tr.onclick=()=>cancelOrder(o,s.scope?.channel);});}
async function cancelOrder(order,channel){if(!confirm(`确认申请撤销Futu订单？\n订单 ${order.channel_order_id}\n${sideLabel(order.side)} ${formatQuantity(order.quantity)}\n虚拟账户 ${channelOrderAccountLabel(order)}`))return;try{const body={account_id:order.account_id,channel_order_id:order.channel_order_id};const endpoint=`/api/channels/${channelSlug(channel)}`;const {token}=await post(`${endpoint}/cancel-token`,body);if(!confirm('撤单令牌已签发，有效期2分钟。再次确认撤单。'))return;await post(`${endpoint}/cancel`,{...body,token});toast('撤单请求已提交');await loadRoute({showLoading:false});}catch(e){toast(e.message);}}
...
async function loadRoute({showLoading=true,forceRender=false}={}){
  ...
  try{
    ...
    else if(route.page==='channel')snapshot=await getJson(`/api/channels/${channelSlug(route.channel)}/snapshot`,{signal:request.signal});
