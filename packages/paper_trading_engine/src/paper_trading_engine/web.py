...
<div class="toolbar"><button id="runSwitch" role="switch" aria-checked="false" onclick="toggleRun()">正在读取运行状态</button></div>
...
<div class="card wide"><div class="label">最新策略决策</div><div class="facts"><span>决策编号</span><span id="decisionId">—</span><span>信号日期</span><span id="signalDate">—</span><span>有效交易日</span><span id="validSession">—</span><span>策略动作</span><span id="action">—</span><span>目标持仓</span><span id="target">—</span><span>执行参考价</span><span id="referencePrice">—</span><span>可部署现金</span><span id="deployableCash">—</span><span>执行费率</span><span id="feeRate">—</span><span>预计订单成本</span><span id="estimatedCost">—</span><span>预计剩余现金</span><span id="remainingCash">—</span></div></div>
...
let lastState=null;
function render(s){lastState=s;const a=s.account||{},d=s.last_decision||{};set('health',cn(s.quote_health));set('environment',`${cn(s.environment)} / ${cn(s.market)}`);set('symbol',s.symbol);set('position',qty(s.actual_quantity));set('runState',s.paused?'已暂停':'自动运行');set('cash',money(a.cash));set('assets',money(a.total_assets));set('frozen',money(a.frozen_cash));set('decisionId',d.decision_id);set('signalDate',d.signal_date);set('validSession',d.valid_session);set('action',cn(d.action));set('target',qty(d.target_quantity));set('referencePrice',money(d.execution_reference_price));set('deployableCash',money(d.available_cash));set('feeRate',d.fee_rate==null?'—':`${(Number(d.fee_rate)*100).toFixed(4)}%`);set('estimatedCost',money(d.estimated_order_cost));set('remainingCash',money(d.unallocated_cash));renderOrders(s.orders);renderList('alerts',s.alerts,x=>cn(x));renderList('events',s.events,e=>`${e.created_at} · ${cn(e.event_type)}`);set('diagnostics',JSON.stringify(s,null,2));const sw=document.getElementById('runSwitch');sw.setAttribute('aria-checked',String(!s.paused));sw.textContent=s.paused?'已暂停 · 点击恢复运行':'自动运行 · 点击暂停新订单';sw.disabled=false}
async function refresh(){try{render(await api('/api/status'))}catch(e){showToast(`状态刷新失败：${e.message}`,true)}}
async function toggleRun(){if(!lastState)return;const sw=document.getElementById('runSwitch'),paused=!!lastState.paused,path=paused?'/api/resume':'/api/pause',success=paused?'恢复成功':'暂停成功';sw.disabled=true;try{render(await api(path,{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'}));showToast(success)}catch(e){showToast(`操作失败：${e.message}`,true);await refresh()}finally{sw.disabled=false}}
