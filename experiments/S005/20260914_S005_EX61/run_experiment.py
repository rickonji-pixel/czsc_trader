...
EXPERIMENT_ID = "20260914_S005_EX61"
...
def main() -> None:
    ...
    if protocol.get("experiment_id") != EXPERIMENT_ID or protocol.get("reads_post_event_returns"):
        raise ValueError("EX61 must match the frozen return-free protocol")
    if any(protocol.get(key) for key in ("candidate_generation", "promotion_allowed", "mutates_strategy_manager", "mutates_pte")):
        raise ValueError("EX61 cannot create, promote, or deploy a candidate")
    ...
    for group_name, group_factor_ids in protocol["factor_groups"].items():
        column = f"{group_name}_score"
        panel[column] = percentile[group_factor_ids].mean(axis=1)
        score_columns.append(column)
    panel["demand_conversion_score"] = panel[score_columns].mean(axis=1)
    group_gate = panel[score_columns].ge(float(protocol["group_floor"])).all(axis=1)

    density_rows: list[dict[str, object]] = []
    event_frames: list[pd.DataFrame] = []
    eligible_start: pd.Timestamp | None = None
    for quantile in protocol["score_quantiles"]:
        label = f"DEMAND_CONVERSION_Q{int(round(float(quantile) * 100))}"
        threshold = panel["demand_conversion_score"].shift(1).rolling(history, min_periods=history).quantile(float(quantile))
        valid = threshold.notna() & panel[score_columns].notna().all(axis=1)
        raw = group_gate & panel["demand_conversion_score"].ge(threshold) & valid
        ...
        rolling = events.loc[valid].astype(int).rolling(
            int(protocol["density"]["window_sessions"]),
            min_periods=int(protocol["density"]["window_sessions"]),
        ).sum().dropna()
        ...
        event_rows = panel.loc[events & valid, score_columns + ["demand_conversion_score"]].reset_index(names="event_date")
    ...
    decision = "STOP_DEMAND_CONVERSION_ON_DENSITY" if selected_mechanism is None else "PROCEED_TO_FIXED_DEMAND_CONVERSION_RETURN_TEST"
    evidence = {
        "schema_version": 1,
    ...
    panel.reset_index(names="first_usable_date").to_csv(
        artifacts / "demand_conversion_panel.csv.gz", index=False, compression=compression, lineterminator="\n"
    )
