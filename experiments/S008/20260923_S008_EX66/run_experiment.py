...
def _evaluate_worker(task: Mapping[str, Any]) -> dict[str, object]:
    ...
    candidate = _candidate(
        ...
        f"EX66T{int(task['trial_number']):06d}",
        parameters,
    )
...
def _safe_reset(path: Path, parent: Path) -> None:
    target = path.resolve()
    root = parent.resolve()
    if target.parent != root or not target.name.startswith("S008_EX66_"):
        raise ValueError("unsafe EX66 temporary path")
    if target.exists():
...
def main() -> None:
    ...
    frozen_paths = {
        ...
        "ex65_manifest_sha256": repo / "experiments/S008/20260923_S008_EX65/experiment_manifest.json",
    ...
    validate_experiment_archive(repo / "experiments/S008/20260923_S008_EX64")
    validate_experiment_archive(repo / "experiments/S008/20260923_S008_EX65")
    ...
    prepared_root = tmp_parent / "S008_EX66_prepared"
    evaluation_root = tmp_parent / "S008_EX66_evaluation"
    _safe_reset(prepared_root, tmp_parent)
    _safe_reset(evaluation_root, tmp_parent)
    anchor_candidate = _candidate(runtime_root, runtime_identity, "EX66ANCHOR", _anchor())
    preparation_instance = StrategyRuntime().create(
        StrategyInit(
            anchor_candidate,
            TradableWindow(
                pd.Timestamp(protocol["preparation_window_start"]).date(),
                evaluation_end.date(),
            ),
            prepared_root,
        )
    )
    preparation_summary = preparation_instance.prepare_data()
    preparation_history = preparation_instance.inspect_signals()
    ready = preparation_history[
        ["gold_silver_ratio_distance_120", "gold_minus_silver_return_20"]
    ].notna().all(axis=1)
    if not ready.any():
        raise ValueError("full development instance has no ready feature session")
    first_ready_signal = pd.Timestamp(ready.index[ready][0]).normalize()
    execution_daily = utilities._load_execution_daily(
        repo,
        repo / "data/raw/518880_execution_manifest.json",
        evaluation_end,
    )
    execution_index = pd.DatetimeIndex(execution_daily["dt"]).normalize()
    signal_position = int(execution_index.get_indexer([first_ready_signal])[0])
    if signal_position < 0 or signal_position + 1 >= len(execution_index):
        raise ValueError("first ready signal has no next execution session")
    evaluation_start = pd.Timestamp(execution_index[signal_position + 1]).normalize()
    ...
    bundle = {
        ...
        "worker_data_dir": str(tmp_parent / "S008_EX66_worker"),
    }
    _worker_init(bundle)
    ...
    buyhold_candidate = _candidate(runtime_root, runtime_identity, "EX66BUYHOLD", _anchor())
    baseline = {
    ...
    data_identity = {
        ...
        "preparation_data_identity": preparation_summary.data_identity,
        "first_ready_signal_session": first_ready_signal.date().isoformat(),
        "input_identities": dict(base_data.input_identities),
        "price_identities": dict(base_data.price_identities),
            ...
            for trial, parameters, failures in sorted(batch, key=lambda item: item[0].number):
                if failures:
                    ...
                    try:
                        ...
                        raise RuntimeError(f"EX66 trial {trial.number} failed") from exc
                rows.append(row)
            _checkpoint(ledger_path, rows)
