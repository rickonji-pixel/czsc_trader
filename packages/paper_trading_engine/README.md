...
PTE不负责行情或策略支持数据发布、策略计算、价格调整、目标仓位或委托数量计算，这些
事实由SRT提供。
...
- `scheduler.py`：观察SRT发布代次、退避、逐账户决策及订单/账户对账节奏；
...
调度器只有在全部活跃标的均生成目标截止日的数据代次后才登记发布成功，只有全部到期账户
...
需要额外历史时点信息的策略，由对应SRT发布器按策略发布ID维护独立支持面板。支持数据与
普通市场数据必须在同一generation中到达同一截止日，任一缺失或哈希不符时PTE都不生成
