...
EXPERIMENT_ID = "20260911_S003_EX45"
...
def main() -> None:
    ...
    if any(
        ...
        raise ValueError("EX45 may only evaluate its frozen mechanism")
    ...
    (experiment / "03_execution.md").write_text(
        "# S003 EX45 执行\n\n"
        f"收益评价结果：`{status}`。{int(row['episodes'])}笔事件，毛收益均值"
        f"{row['gross_mean_return']:.3%}、基准均值{row['baseline_mean_return']:.3%}、压力均值"
