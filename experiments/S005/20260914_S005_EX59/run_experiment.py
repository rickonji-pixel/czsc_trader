...
EXPERIMENT_ID = "20260914_S005_EX59"
...
def _historical_percentile(series: pd.Series, history: int) -> pd.Series:
    ...
    return values.rolling(history + 1, min_periods=history + 1).apply(
...
def main() -> None:
    ...
    if protocol.get("reads_post_event_returns"):
        raise ValueError("EX59 is a return-free density calibration")
    if any(protocol.get(key) for key in ("candidate_generation", "promotion_allowed", "mutates_strategy_manager", "mutates_pte")):
        raise ValueError("EX59 cannot create, promote, or deploy a candidate")
    ...
    for path, expected in (
        ...
        if _sha256(path) != expected:
            raise ValueError(f"repaired catalog differs: {path}")
    ...
    factor_ids = [item for values in protocol["factor_groups"].values() for item in values]
    ...
    for factor_id in factor_ids:
        source_values = panel[factor_id].abs() if factor_id == "F-PROJECT-ETF-NAV-PREMIUM" else panel[factor_id]
        percentile[factor_id] = _historical_percentile(source_values, history)
    panel["component_participation_score"] = percentile[protocol["factor_groups"]["component_participation"]].mean(axis=1)
    panel["component_flow_score"] = percentile["F-PROJECT-MONEYFLOW-BREADTH"]
    panel["primary_market_score"] = percentile["F-PROJECT-ETF-SHARE-CHANGE"]
    panel["pricing_efficiency_score"] = 1.0 - percentile["F-PROJECT-ETF-NAV-PREMIUM"]
    score_columns = [
        "component_participation_score",
        "component_flow_score",
        "primary_market_score",
        "pricing_efficiency_score",
    ]
    panel["coordinated_demand_score"] = panel[score_columns].mean(axis=1)

    density_rows: list[dict[str, object]] = []
    event_frames: list[pd.DataFrame] = []
    eligible_start: pd.Timestamp | None = None
    for quantile in protocol["score_quantiles"]:
        label = f"COORDINATED_DEMAND_Q{int(round(float(quantile) * 100))}"
        threshold = panel["coordinated_demand_score"].shift(1).rolling(history, min_periods=history).quantile(float(quantile))
        raw = panel["coordinated_demand_score"].ge(threshold) & threshold.notna()
        ...
        rolling = events.loc[valid].astype(int).rolling(int(protocol["density"]["window_sessions"]), min_periods=int(protocol["density"]["window_sessions"])).sum().dropna()
        ...
        selected_events = panel.loc[events & valid, score_columns + ["coordinated_demand_score"]].reset_index(names="event_date")
        selected_events.insert(0, "mechanism", label)
        selected_events["threshold"] = threshold.loc[events & valid].to_numpy()
    ...
    decision = "STOP_COORDINATED_DEMAND_ON_DENSITY" if selected_mechanism is None else "PROCEED_TO_FIXED_COORDINATED_DEMAND_RETURN_TEST"
    evidence = {
        "schema_version": 1,
        ...
        "information_families": ["MARKET_BREADTH", "FUND_FLOW", "POSITION_VALUATION"],
    ...
    output = panel.reset_index(names="first_usable_date")
    compression = {"method": "gzip", "compresslevel": 9, "mtime": 0}
    output.to_csv(artifacts / "coordinated_demand_panel.csv.gz", index=False, compression=compression, lineterminator="\n")
    pd.concat(event_frames, ignore_index=True).to_csv(artifacts / "event_ledger.csv", index=False, encoding="utf-8-sig", lineterminator="\n")
    ...
    for row in density.itertuples(index=False):
        table.append(f"|{row.mechanism}|{row.independent_events}|{row.rolling_60_median:.0f}/{row.rolling_60_p10:.0f}|{row.rolling_60_min:.0f}/{row.rolling_60_max:.0f}|{'PASS' if row.density_pass else 'FAIL'}|")
    (experiment / "03_execution.md").write_text("# S005 EX59 执行\n\n状态：`COMPLETE`。项目级协同需求机制完成无收益密度校准。\n\n" + "\n".join(table) + "\n", encoding="utf-8")
    (experiment / "04_conclusion.md").write_text(
        "# S005 EX59 结论\n\n" + "\n".join(table) + "\n\n"
        f"裁决：`{decision}`。按事前规则获得唯一收益检验资格的路径：{selected_mechanism or 'NONE'}。"
        "本轮只确认事件供给，不代表存在Alpha；没有读取事件后收益，也没有生成候选或修改SM/PTE。\n",
        encoding="utf-8",
    )
