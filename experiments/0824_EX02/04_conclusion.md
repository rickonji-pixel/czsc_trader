...
5. 唯一达到4/5年度稳定局部改善要求的规则变化，是将入场确认从1个交易日改为2个交易日。它识别了当前即时入场规则的负向方向，但尚不能证明2日就是最优值，也没有形成新冠军。
...
各年度最大单一持仓差异区间占比均低于50%，上述4/5年度改善不受预注册的单一区间主导规则否决。因此，下一轮最有证据的单一挑战假设是“入场信号需要连续2日确认”，而不是删除某个因子、改变权重或调整阈值。
