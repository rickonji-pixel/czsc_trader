...
from strategy_runtime.strategy import StrategyInstance


EXPERIMENT_ID = "20260923_S008_EX24"
...
def _managed_directory(*, prefix: str, dir: Path):
    ...
    path = (root / "S008_EX24_runtime").resolve()
    if path.parent != root or path.name != "S008_EX24_runtime":
        raise ValueError("unsafe EX24 temporary path")
    if path.exists():
        shutil.rmtree(path)
...
def main() -> None:
    ...
    predecessor = repo / "experiments/S008/20260923_S008_EX23"
    base = repo / "experiments/S008/20260923_S008_EX20"
    ...
    paths = {
        "ex23_manifest_sha256": predecessor / "experiment_manifest.json",
    ...
    original_prepare = StrategyInstance.prepare_data

    def prepare_with_input_count(instance):
        original_prepare(instance)
        return SimpleNamespace(inputs=instance.definition.inputs.requirements)

    with mock_patch.object(StrategyInstance, "prepare_data", prepare_with_input_count):
        module.main()

    for name in ("03_execution.md", "04_conclusion.md"):
        path = experiment / name
        path.write_text(
            path.read_text(encoding="utf-8").replace("# S008 EX20", "# S008 EX24"),
            encoding="utf-8",
        )
