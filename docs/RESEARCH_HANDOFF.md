...
本文档记录截至 **2026-08-25 / 0825_EX03** 的可接力状态。新会话应先验证仓库当前状态，不能把本文中的分支或提交描述当成未经核验的实时事实。
...
| `scripts/run_experiment.py` | 按实验目录协议运行早期冠军挑战、归因诊断、0825_EX02或0825_EX03 |
...
EX03—0825_EX03的运行器默认绑定各自实验目录。已有实验是冻结档案，不应为“复现”而直接覆盖运行。若要开展新实验，先创建新的 `MMDD_EXX` 目录并预注册协议，再显式传入 `--experiment-dir`。
...
EX08和0825_EX01已经拒绝“扩大同一搜索或重排同一Trial集合”。0825_EX02说明EX04附近是局部平台；0825_EX03进一步拒绝“统一放宽入场”或“把退出整体改回历史冠军”这两种简单修复。三个落后窗口的主导路径不同，退出侧回退虽修复三个落后窗口，却伤害六个对照窗口。

下一步优先级：

1. 暂停直接全局优化，先预注册新的退出侧诊断：解释2021H1、2023H1的持仓中断收益为何不能迁移到多数对照窗口，并检查收益是否集中在少数区间；
2. 只有退出侧的条件机制得到跨窗口证据后，才单独设计条件化退出改动；不得直接照搬`baseline_20260823`退出阈值，也不得同轮混入2025H2的入场修复；
3. 2025H2迟入场是另一条独立假设，只有退出侧诊断不能形成稳定机制时再单独预注册，不与退出改动同时搜索；
4. 项目允许588080使用标的独立策略，因此当前不要求先做跨标的外推；但仍要求同一标的内控制窗口不被系统性破坏；
5. 保持四层因果执行和EX04公平研究基线，不再以局部权重、阈值微调、删除因子组或增加Trial作为单一变化；
...
5. 验证0824_EX01—0825_EX03实验清单，不依赖历史outputs。
...
重要边界：baseline_20260823是通用回测正式基线；EX04冻结策略只是EX05以后使用的研究基线。EX07完成608个SQLite Optuna Trial，EX08完成4096个纯内存Optuna Trial；0825_EX01复用EX08输出按三套规则冻结8个唯一候选并统一补测2026，三轮挑战均FAIL。0825_EX02和0825_EX03只用2020—2025完成EX04机制及交易路径归因，均为COMPLETE、不判PASS/FAIL、不访问2026；EX03主分类为mixed_path_failure，不能统一放宽入场或整体回退退出逻辑。研究事实只认Git跟踪的experiments档案；outputs只用于当前设备新运行的普通回测。2026约束按单次实验执行：策略冻结前不可访问，冻结后一次验收，不得把验收结果反馈给同轮参数。
